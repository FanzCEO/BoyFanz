...
// ===== FUCK BUDDIES SOCIAL CONNECTIONS =====
// Adult social networking - like Facebook friends but naughty

export const fuckBuddyRelationshipEnum = pgEnum("fuck_buddy_relationship", [
  "fuckbuddy",
  "fwb",
  "crush",
  "lover",
...
export const fuckBuddyRequestStatusEnum = pgEnum("fuck_buddy_request_status", [
  "pending",
  "accepted",
  "declined",
  "blocked",
...
// Fuck buddy requests (friend requests)
export const fuckBuddyRequests = pgTable(
  "fuck_buddy_requests",
  {
    id: varchar("id")
      .primaryKey()
    ...
    receiverId: varchar("receiver_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    relationshipType: fuckBuddyRelationshipEnum("relationship_type").default("fuckbuddy").notNull(),
    message: text("message"), // optional message with request
    status: fuckBuddyRequestStatusEnum("status").default("pending").notNull(),
    respondedAt: timestamp("responded_at"),
    createdAt: timestamp("created_at").defaultNow(),
  },
  (table) => [
    index("idx_fuck_buddy_requests_sender").on(table.senderId),
    index("idx_fuck_buddy_requests_receiver").on(table.receiverId),
    index("idx_fuck_buddy_requests_status").on(table.status),
    unique("unique_fuck_buddy_request").on(table.senderId, table.receiverId),
  ],
);

// Fuck buddies connections (established friendships)
export const fuckBuddies = pgTable(
  "fuck_buddies",
  {
    id: varchar("id")
      .primaryKey()
    ...
    userId: varchar("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    buddyId: varchar("buddy_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    relationshipType: fuckBuddyRelationshipEnum("relationship_type").default("fuckbuddy").notNull(),
    nickname: varchar("nickname"), // custom name for this buddy
    isTopEight: boolean("is_top_eight").default(false), // MySpace Top 8 style!
    topEightPosition: integer("top_eight_position"), // 1-8
    connectionScore: integer("connection_score").default(0), // interaction frequency
    connectedAt: timestamp("connected_at").defaultNow(),
    lastInteractionAt: timestamp("last_interaction_at").defaultNow(),
  },
  (table) => [
    index("idx_fuck_buddies_user").on(table.userId),
    index("idx_fuck_buddies_buddy").on(table.buddyId),
    index("idx_fuck_buddies_top_eight").on(table.userId, table.isTopEight),
    unique("unique_fuck_buddy_pair").on(table.userId, table.buddyId),
  ],
);

// Social notifications for buddy activities
export const socialNotifications = pgTable(
  "social_notifications",
  {
    ...
    type: varchar("type").notNull(), // buddy_request, buddy_accepted, buddy_top_eight, etc.
    title: varchar("title").notNull(),
    message: text("message"),
  ...
  (table) => [
    index("idx_social_notifications_user").on(table.userId),
    index("idx_social_notifications_unread").on(table.userId, table.isRead),
    index("idx_social_notifications_created").on(table.createdAt.desc()),
  ],
);

// Relations
export const fuckBuddyRequestsRelations = relations(fuckBuddyRequests, ({ one }) => ({
  sender: one(users, {
    fields: [fuckBuddyRequests.senderId],
    references: [users.id],
    relationName: "sentBuddyRequests",
  }),
  receiver: one(users, {
    fields: [fuckBuddyRequests.receiverId],
    references: [users.id],
    relationName: "receivedBuddyRequests",
  }),
}));

export const fuckBuddiesRelations = relations(fuckBuddies, ({ one }) => ({
  user: one(users, {
    fields: [fuckBuddies.userId],
    references: [users.id],
    relationName: "userBuddies",
  }),
  buddy: one(users, {
    fields: [fuckBuddies.buddyId],
    references: [users.id],
    relationName: "buddyOf",
  }),
}));
...
export const socialNotificationsRelations = relations(socialNotifications, ({ one }) => ({
  ...
  fromUser: one(users, {
    fields: [socialNotifications.fromUserId],
    references: [users.id],
  }),
}));

// Types
export type FuckBuddyRequest = typeof fuckBuddyRequests.$inferSelect;
export type InsertFuckBuddyRequest = typeof fuckBuddyRequests.$inferInsert;
export type FuckBuddy = typeof fuckBuddies.$inferSelect;
export type InsertFuckBuddy = typeof fuckBuddies.$inferInsert;
...
// ===== NAUGHTY PROFILE EXTENSIONS =====

// Extended user profiles for naughty profile features
export const userProfiles = pgTable(
  "user_profiles",
  {
    ...
    fantasies: jsonb("fantasies").$type<string[]>().default([]),
    kinks: jsonb("kinks").$type<string[]>().default([]),
    turnOns: jsonb("turn_ons").$type<string[]>().default([]),
    turnOffs: jsonb("turn_offs").$type<string[]>().default([]),
    lookingFor: jsonb("looking_for").$type<string[]>().default([]),
    sexualOrientation: varchar("sexual_orientation"),
    ...
    bio: text("bio"),
    styleAttitude: jsonb("style_attitude").$type<string[]>().default([]),
    profileTheme: jsonb("profile_theme"),
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
  },
  (table) => [
...
// Wall posts (MySpace-style profile wall)
export const wallPosts = pgTable(
  "wall_posts",
  {
    ...
    authorId: varchar("author_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    type: varchar("type").default("text").notNull(), // text, media, mood
    content: text("content"),
    mediaUrls: jsonb("media_urls").$type<string[]>().default([]),
    mood: varchar("mood"),
    moodEmoji: varchar("mood_emoji"),
    location: varchar("location"),
  ...
  (table) => [
    index("idx_wall_posts_profile").on(table.profileUserId),
    index("idx_wall_posts_author").on(table.authorId),
    index("idx_wall_posts_pinned").on(table.profileUserId, table.isPinned),
    index("idx_wall_posts_created").on(table.createdAt.desc()),
  ],
);

// Wall post reactions
export const wallReactions = pgTable(
  "wall_reactions",
  {
    id: varchar("id")
      .primaryKey()
    ...
    userId: varchar("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    reactionType: varchar("reaction_type").notNull(), // fire, heart, eggplant, peach, eyes, devil
    createdAt: timestamp("created_at").defaultNow(),
  },
  (table) => [
    index("idx_wall_reactions_post").on(table.postId),
    index("idx_wall_reactions_user").on(table.userId),
    unique("unique_wall_reaction").on(table.postId, table.userId, table.reactionType),
  ],
);
...
export const testimonials = pgTable(
  ...
  (table) => [
    index("idx_testimonials_to_user").on(table.toUserId),
    index("idx_testimonials_from_user").on(table.fromUserId),
    unique("unique_testimonial").on(table.toUserId, table.fromUserId),
  ],
);

// Relations for naughty profile tables
...
export const wallPostsRelations = relations(wallPosts, ({ one, many }) => ({
  profileUser: one(users, {
    fields: [wallPosts.profileUserId],
    references: [users.id],
    relationName: "wallPostsOnProfile",
  }),
  author: one(users, {
    fields: [wallPosts.authorId],
    references: [users.id],
    relationName: "wallPostsAuthored",
  }),
...
export const testimonialsRelations = relations(testimonials, ({ one }) => ({
  toUser: one(users, {
    fields: [testimonials.toUserId],
    references: [users.id],
    relationName: "receivedTestimonials",
  }),
  fromUser: one(users, {
    fields: [testimonials.fromUserId],
    references: [users.id],
    relationName: "writtenTestimonials",
  }),
}));

// Types for naughty profile
...
// ===== SECTION 7: ADMIN PANEL & ANALYTICS SCHEMA =====
// Comprehensive admin management, analytics, and platform intelligence

// Admin action audit trail
export const adminActionType = pgEnum('admin_action_type', [
  'user_suspend', 'user_unsuspend', 'user_delete', 'user_role_change',
  'content_remove', 'content_approve', 'content_feature',
  'payout_approve', 'payout_reject', 'refund_issue',
  'verification_approve', 'verification_reject',
  'setting_change', 'feature_toggle', 'mass_action',
  'impersonation_start', 'impersonation_end',
  'announcement_publish', 'complaint_resolve'
]);
...
export const adminAuditLog = pgTable('admin_audit_log', {
  ...
  index('idx_admin_audit_target').on(table.targetTable, table.targetId),
  index('idx_admin_audit_created').on(table.createdAt)
]);

// Creator health scores - AI-powered account assessment
export const creatorHealthScores = pgTable('creator_health_scores', {
  ...
  riskFactors: jsonb('risk_factors').default([]), // Array of risk identifiers
  recommendations: jsonb('recommendations').default([]), // AI suggestions
  trendDirection: varchar('trend_direction').default('stable'), // up, down, stable
  trendPercentage: decimal('trend_percentage', { precision: 5, scale: 2 }).default('0'),
  lastAnalyzedAt: timestamp('last_analyzed_at').defaultNow(),
  aiModelVersion: varchar('ai_model_version').default('v1'),
  rawMetrics: jsonb('raw_metrics').default({}),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow()
}, (table) => [
  index('idx_creator_health_creator').on(table.creatorId),
  index('idx_creator_health_score').on(table.overallScore),
  index('idx_creator_health_trend').on(table.trendDirection)
]);

// Fraud detection signals
export const fraudSignalType = pgEnum('fraud_signal_type', [
  'suspicious_signup', 'payment_fraud', 'fake_engagement', 'account_takeover',
  'chargeback_pattern', 'bot_activity', 'identity_mismatch', 'velocity_abuse',
  'referral_fraud', 'multi_accounting', 'content_theft', 'suspicious_payout'
]);

export const fraudSignalSeverity = pgEnum('fraud_signal_severity', [
...
export const fraudSignals = pgTable('fraud_signals', {
  ...
  signalType: fraudSignalType('signal_type').notNull(),
  severity: fraudSignalSeverity('severity').notNull(),
  confidence: decimal('confidence', { precision: 5, scale: 2 }).notNull(), // 0-100
  description: text('description').notNull(),
  evidence: jsonb('evidence').default({}),
  relatedSignals: text('related_signals').array().default([]),
  status: varchar('status').default('active'), // active, dismissed, confirmed, resolved
  reviewedBy: varchar('reviewed_by').references(() => users.id),
  reviewedAt: timestamp('reviewed_at'),
  resolution: text('resolution'),
  automatedAction: varchar('automated_action'), // Action taken automatically
  financialImpactCents: integer('financial_impact_cents').default(0),
  ipAddresses: text('ip_addresses').array().default([]),
  deviceFingerprints: text('device_fingerprints').array().default([]),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow()
}, (table) => [
  index('idx_fraud_signals_user').on(table.userId),
  index('idx_fraud_signals_type').on(table.signalType),
  index('idx_fraud_signals_severity').on(table.severity),
  index('idx_fraud_signals_status').on(table.status),
  index('idx_fraud_signals_created').on(table.createdAt)
]);

// Platform A/B testing experiments
...
export const experiments = pgTable('experiments', {
  ...
  featureFlag: varchar('feature_flag').notNull(),
  variants: jsonb('variants').default([]), // Array of variant configs
  ...
  secondaryMetrics: text('secondary_metrics').array().default([]),
  minimumSampleSize: integer('minimum_sample_size').default(1000),
  currentSampleSize: integer('current_sample_size').default(0),
  statisticalSignificance: decimal('statistical_significance', { precision: 5, scale: 2 }),
  ...
  index('idx_experiments_feature').on(table.featureFlag)
]);
...
export const experimentAssignments = pgTable('experiment_assignments', {
  ...
  index('idx_exp_assign_experiment').on(table.experimentId),
  index('idx_exp_assign_user').on(table.userId),
  unique().on(table.experimentId, table.userId)
]);

// Platform settings with versioning
...
// Compliance reports (2257, GDPR, tax)
export const complianceReportType = pgEnum('compliance_report_type', [
  '2257_records', 'gdpr_data_export', 'gdpr_deletion', 'tax_1099',
  'tax_summary', 'content_audit', 'user_activity', 'financial_summary'
]);

export const complianceReports = pgTable('compliance_reports', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  reportType: complianceReportType('report_type').notNull(),
  title: varchar('title').notNull(),
  description: text('description'),
  periodStart: timestamp('period_start'),
  periodEnd: timestamp('period_end'),
  status: varchar('status').default('generating'), // generating, ready, expired, error
  fileUrl: varchar('file_url'),
  fileSize: integer('file_size'),
  recordCount: integer('record_count'),
  generatedBy: varchar('generated_by').notNull().references(() => users.id),
  scheduledExpiryAt: timestamp('scheduled_expiry_at'),
  metadata: jsonb('metadata').default({}),
  createdAt: timestamp('created_at').defaultNow(),
  completedAt: timestamp('completed_at')
}, (table) => [
  index('idx_compliance_reports_type').on(table.reportType),
  index('idx_compliance_reports_status').on(table.status)
]);

// Real-time platform metrics cache
export const platformMetrics = pgTable('platform_metrics', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  metricName: varchar('metric_name').notNull(),
  metricValue: decimal('metric_value', { precision: 20, scale: 4 }).notNull(),
  dimension: varchar('dimension'), // Optional dimension (e.g., country, category)
  ...
  index('idx_platform_metrics_period').on(table.period, table.periodStart),
  unique().on(table.metricName, table.period, table.periodStart, table.dimension, table.dimensionValue)
]);
...
export const leaderboardCategory = pgEnum('leaderboard_category', [
  'earnings', 'subscribers', 'content_views', 'engagement', 'tips', 'new_subscribers', 'growth_rate'
]);

export const creatorLeaderboards = pgTable('creator_leaderboards', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  creatorId: varchar('creator_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  category: leaderboardCategory('category').notNull(),
  period: leaderboardPeriod('period').notNull(),
  periodStart: timestamp('period_start').notNull(),
  rank: integer('rank').notNull(),
  previousRank: integer('previous_rank'),
  rankChange: integer('rank_change').default(0),
  score: decimal('score', { precision: 20, scale: 4 }).notNull(),
  displayValue: varchar('display_value'), // Formatted value for display
  percentile: decimal('percentile', { precision: 5, scale: 2 }),
  isEligible: boolean('is_eligible').default(true),
  metadata: jsonb('metadata').default({}),
  createdAt: timestamp('created_at').defaultNow()
}, (table) => [
  index('idx_leaderboard_creator').on(table.creatorId),
  index('idx_leaderboard_category_period').on(table.category, table.period, table.periodStart),
  index('idx_leaderboard_rank').on(table.category, table.period, table.periodStart, table.rank)
]);

// Financial reconciliation
export const reconciliationStatus = pgEnum('reconciliation_status', [
  'pending', 'matched', 'discrepancy', 'resolved', 'manual_review'
]);

export const financialReconciliation = pgTable('financial_reconciliation', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  recordDate: timestamp('record_date').notNull(),
  source: varchar('source').notNull(), // stripe, paypal, internal, etc.
  externalRef: varchar('external_ref'),
  internalRef: varchar('internal_ref'),
  expectedAmountCents: integer('expected_amount_cents').notNull(),
  actualAmountCents: integer('actual_amount_cents'),
  discrepancyCents: integer('discrepancy_cents').default(0),
  status: reconciliationStatus('status').default('pending').notNull(),
  ...
  index('idx_reconciliation_date').on(table.recordDate),
  index('idx_reconciliation_status').on(table.status),
  index('idx_reconciliation_source').on(table.source)
]);

// Creator analytics aggregates (for fast dashboard loading)
export const creatorAnalytics = pgTable('creator_analytics', {
  ...
  // Earnings breakdown
  totalEarningsCents: integer('total_earnings_cents').default(0),
  subscriptionEarningsCents: integer('subscription_earnings_cents').default(0),
  tipEarningsCents: integer('tip_earnings_cents').default(0),
  ppvEarningsCents: integer('ppv_earnings_cents').default(0),
  referralEarningsCents: integer('referral_earnings_cents').default(0),
  
  // Subscriber metrics
  totalSubscribers: integer('total_subscribers').default(0),
  newSubscribers: integer('new_subscribers').default(0),
  cancelledSubscribers: integer('cancelled_subscribers').default(0),
  reactivatedSubscribers: integer('reactivated_subscribers').default(0),
  
  // Content metrics
  postsCount: integer('posts_count').default(0),
  messagesCount: integer('messages_count').default(0),
  streamMinutes: integer('stream_minutes').default(0),
  ...
  index('idx_creator_analytics_creator').on(table.creatorId),
  index('idx_creator_analytics_period').on(table.periodType, table.periodStart),
  unique().on(table.creatorId, table.periodType, table.periodStart)
]);


// ===== STARZ STUDIO MEMBERSHIP SYSTEM =====
// AI Creator Studio - Membership earned through performance, not payment
// Criteria: Fan count, referrals, media quality, post volume

// Starz tier levels - each unlocks more AI creator tools
export const starzTierEnum = pgEnum("starz_tier", [
  "none",           // No membership - can view tools only
  "bronze_star",    // Entry level - basic AI tools
  "silver_star",    // Intermediate - enhanced AI features
  "gold_star",      // Advanced - full AI suite access
  "platinum_star",  // Premium - priority AI processing
  "diamond_star",   // Elite - all features + beta access + dedicated support
]);

// Starz Studio membership tracking
export const starzMemberships = pgTable(
  "starz_memberships",
  {
    id: varchar("id")
      .primaryKey()
    ...
    // Current tier
    currentTier: starzTierEnum("current_tier").default("none").notNull(),
    previousTier: starzTierEnum("previous_tier"),
    
    // Tier qualification metrics (real-time)
    totalFanCount: integer("total_fan_count").default(0),
    activeFanCount: integer("active_fan_count").default(0), // Fans active in last 30 days
    successfulReferrals: integer("successful_referrals").default(0),
    pendingReferrals: integer("pending_referrals").default(0),
    mediaQualityScore: integer("media_quality_score").default(0), // 0-100
    totalPostCount: integer("total_post_count").default(0),
    ...
    contentConsistencyScore: integer("content_consistency_score").default(0), // 0-100
    communityHealthScore: integer("community_health_score").default(0), // 0-100
    
    // Tier progress tracking
    tierProgress: jsonb("tier_progress").default({}), // Progress toward next tier
    nextTierRequirements: jsonb("next_tier_requirements").default({}),
    
    // FanzCloud Mobile app access
    fanzcloudAccessEnabled: boolean("fanzcloud_access_enabled").default(false),
    fanzcloudAccessLevel: varchar("fanzcloud_access_level").default("view_only"), // view_only, basic, full
    fanzcloudActivatedAt: timestamp("fanzcloud_activated_at"),
    
    // Timeline
    tierAchievedAt: timestamp("tier_achieved_at").defaultNow(),
    tierExpiresAt: timestamp("tier_expires_at"), // Null = permanent (maintained by performance)
  ...
  (table) => [
    index("idx_starz_memberships_profile").on(table.profileId),
    index("idx_starz_memberships_tier").on(table.currentTier),
    index("idx_starz_memberships_fanzcloud").on(table.fanzcloudAccessEnabled),
  ],
);

// Starz tier requirements configuration
export const starzTierRequirements = pgTable(
  "starz_tier_requirements",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    tier: starzTierEnum("tier").notNull().unique(),
    
    // Fan count requirements
    minFanCount: integer("min_fan_count").notNull(),
    minActiveFanCount: integer("min_active_fan_count").notNull(),
    
    // Referral requirements
    minReferrals: integer("min_referrals").notNull(),
    
    // Media quality requirements (0-100 scale)
    minMediaQualityScore: integer("min_media_quality_score").notNull(),
    
    // Post requirements
    minTotalPosts: integer("min_total_posts").notNull(),
    minMonthlyPosts: integer("min_monthly_posts").notNull(),
    
    // Optional advanced requirements
    minEngagementRate: decimal("min_engagement_rate", { precision: 5, scale: 2 }),
    minConsistencyScore: integer("min_consistency_score"),
    minCommunityHealthScore: integer("min_community_health_score"),
    
    // Feature unlocks at this tier
    features: jsonb("features").default([]),
    aiToolsUnlocked: text("ai_tools_unlocked").array().default([]),
    fanzcloudAccessLevel: varchar("fanzcloud_access_level").notNull(),
    
    // Display information
    displayName: varchar("display_name").notNull(),
    description: text("description"),
    badgeUrl: varchar("badge_url"),
    color: varchar("color"), // Hex color for tier display
    
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
  },
  (table) => [
    index("idx_starz_tier_requirements_tier").on(table.tier),
  ],
);

// Starz referral tracking for tier qualification
export const starzReferralTracking = pgTable(
  "starz_referral_tracking",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    referrerProfileId: varchar("referrer_profile_id")
      .notNull()
      .references(() => profiles.id, { onDelete: "cascade" }),
    ...
    // Qualification status for Starz membership
    qualifiesForStarz: boolean("qualifies_for_starz").default(false),
    qualificationReason: text("qualification_reason"),
    
    // Referred user metrics (for qualification)
    referredUserActive: boolean("referred_user_active").default(false),
    referredUserPurchased: boolean("referred_user_purchased").default(false),
    referredUserSubscribed: boolean("referred_user_subscribed").default(false),
    referredUserValue: decimal("referred_user_value", { precision: 10, scale: 2 }).default("0"),
  ...
  (table) => [
    index("idx_starz_referral_tracking_referrer").on(table.referrerProfileId),
    index("idx_starz_referral_tracking_referred").on(table.referredUserId),
    index("idx_starz_referral_tracking_qualifies").on(table.qualifiesForStarz),
  ],
);

// Media quality scoring for Starz qualification
export const mediaQualityScores = pgTable(
  "media_quality_scores",
  {
    ...
    // Technical quality scores (0-100)
    resolutionScore: integer("resolution_score").default(0),
    bitrateScore: integer("bitrate_score").default(0),
    framingScore: integer("framing_score").default(0),
    lightingScore: integer("lighting_score").default(0),
    audioScore: integer("audio_score").default(0),
    
    // Content quality scores (0-100)
    originalityScore: integer("originality_score").default(0),
    ...
    aiAnalysisResults: jsonb("ai_analysis_results").default({}),
    
    analyzedAt: timestamp("analyzed_at").defaultNow(),
    createdAt: timestamp("created_at").defaultNow(),
  },
  (table) => [
    index("idx_media_quality_scores_media").on(table.mediaAssetId),
    index("idx_media_quality_scores_profile").on(table.profileId),
    index("idx_media_quality_scores_overall").on(table.overallScore),
  ],
);

// Starz tier change history
export const starzTierHistory = pgTable(
  "starz_tier_history",
  {
    id: varchar("id")
      .primaryKey()
    ...
    membershipId: varchar("membership_id")
      .notNull()
      .references(() => starzMemberships.id, { onDelete: "cascade" }),
    
    previousTier: starzTierEnum("previous_tier"),
    newTier: starzTierEnum("new_tier").notNull(),
    changeReason: varchar("change_reason").notNull(), // promotion, demotion, manual_adjustment
    
    // Metrics at time of change
    metricsSnapshot: jsonb("metrics_snapshot").default({}),
    
    changedAt: timestamp("changed_at").defaultNow(),
    changedBy: varchar("changed_by").references(() => accounts.id), // null for automatic
  },
  (table) => [
    index("idx_starz_tier_history_membership").on(table.membershipId),
    index("idx_starz_tier_history_changed_at").on(table.changedAt),
  ],
);

// SSO access logging for Starz Studio tools
export const starzAccessLogs = pgTable(
  "starz_access_logs",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    profileId: varchar("profile_id")
      ...
      .references(() => profiles.id, { onDelete: "cascade" }),
    membershipId: varchar("membership_id")
      .references(() => starzMemberships.id),
    
    // Access details
    toolAccessed: varchar("tool_accessed").notNull(), // AI tool or feature accessed
    accessType: varchar("access_type").notNull(), // view, use, denied
    tierAtAccess: starzTierEnum("tier_at_access"),
    
    // SSO session info
    ssoSessionId: varchar("sso_session_id"),
    ssoClientId: varchar("sso_client_id"), // Which platform accessed from
    
    // Device and location
    ipAddress: inet("ip_address"),
    userAgent: text("user_agent"),
    deviceType: varchar("device_type"), // web, mobile, fanzcloud_app
    
    // FanzCloud specific
    fanzcloudVersion: varchar("fanzcloud_version"),
    fanzcloudPlatform: varchar("fanzcloud_platform"), // ios, android
    
    accessedAt: timestamp("accessed_at").defaultNow().notNull(),
  },
  (table) => [
    index("idx_starz_access_logs_profile").on(table.profileId),
    index("idx_starz_access_logs_tool").on(table.toolAccessed),
    index("idx_starz_access_logs_access_type").on(table.accessType),
    index("idx_starz_access_logs_accessed_at").on(table.accessedAt),
    index("idx_starz_access_logs_sso_session").on(table.ssoSessionId),
  ],
);

// AI tools catalog for Starz Studio
export const starzAiTools = pgTable(
  "starz_ai_tools",
  {
    id: varchar("id")
      .primaryKey()
    ...
    name: varchar("name").notNull(),
    description: text("description"),
    category: varchar("category").notNull(), // content, analytics, engagement, automation
    
    // Tier requirements
    minimumTier: starzTierEnum("minimum_tier").notNull(),
    
    // Usage limits per tier
    usageLimits: jsonb("usage_limits").default({}), // { bronze_star: 10, silver_star: 50, ... }
    
    // Tool configuration
    apiEndpoint: varchar("api_endpoint"),
    aiModel: varchar("ai_model"), // gpt-4o, claude-3, custom
    parameters: jsonb("parameters").default({}),
  ...
  (table) => [
    index("idx_starz_ai_tools_slug").on(table.slug),
    index("idx_starz_ai_tools_category").on(table.category),
    index("idx_starz_ai_tools_tier").on(table.minimumTier),
  ],
);

// FanzCloud Mobile app sessions (tracked via SSO)
export const fanzcloudSessions = pgTable(
  "fanzcloud_sessions",
  {
    id: varchar("id")
      .primaryKey()
    ...
    // SSO authentication
    ssoAccessToken: varchar("sso_access_token"),
    ssoRefreshToken: varchar("sso_refresh_token"),
    ssoExpiresAt: timestamp("sso_expires_at"),
  ...
  (table) => [
    index("idx_fanzcloud_sessions_profile").on(table.profileId),
    index("idx_fanzcloud_sessions_device").on(table.deviceId),
    index("idx_fanzcloud_sessions_active").on(table.isActive),
    unique().on(table.profileId, table.deviceId),
  ],
);


// ===== STARZ AUXILIARY PLATFORM CONNECTORS =====
// Cross-platform connections available to Starz Studio members
// Enables automatic cross-posting, analytics aggregation, and unified management

export const auxiliaryPlatformEnum = pgEnum("auxiliary_platform", [
  "twitter",
  "instagram",
  "tiktok",
  "youtube",
  ...
  "snapchat",
  "twitch",
  "pornhub",    // Adult platform integrations
  "xvideos",
  "manyvids",
  "chaturbate",
  "stripchat",
  "fansly",
  "onlyfans",   // For migration tools
  "linktree",
  "allmylinks",
  "beacons",
]);
...
// Starz Platform Connections - OAuth connections to external platforms
export const starzPlatformConnections = pgTable(
  "starz_platform_connections",
  {
    ...
    // Platform details
    platform: auxiliaryPlatformEnum("platform").notNull(),
    ...
    platformProfileUrl: varchar("platform_profile_url"),
    platformAvatarUrl: varchar("platform_avatar_url"),
    
    // OAuth tokens (encrypted)
    accessToken: text("access_token"),
    refreshToken: text("refresh_token"),
    tokenExpiresAt: timestamp("token_expires_at"),
    tokenScopes: text("token_scopes").array().default([]),
    
    // Connection status
    status: platformConnectionStatusEnum("status").default("pending").notNull(),
    lastSyncAt: timestamp("last_sync_at"),
    ...
    importEnabled: boolean("import_enabled").default(false), // For content migration
    
    // Platform-specific settings
    settings: jsonb("settings").default({}),
    
    // Minimum tier required (some platforms need higher tiers)
    minimumTier: starzTierEnum("minimum_tier").default("bronze_star"),
    
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
  },
  (table) => [
    unique().on(table.profileId, table.platform),
    index("idx_starz_platform_connections_profile").on(table.profileId),
    index("idx_starz_platform_connections_platform").on(table.platform),
    index("idx_starz_platform_connections_status").on(table.status),
  ],
);

// Cross-post queue for scheduled posts to auxiliary platforms
...
export const starzCrossPosts = pgTable(
  "starz_cross_posts",
  {
    id: varchar("id")
      .primaryKey()
    ...
    connectionId: varchar("connection_id")
      .notNull()
      .references(() => starzPlatformConnections.id, { onDelete: "cascade" }),
    contentId: varchar("content_id")
    ...
    platformCaption: text("platform_caption"), // AI-adapted caption for platform
    platformHashtags: text("platform_hashtags").array().default([]),
    platformMediaUrls: text("platform_media_urls").array().default([]), // Resized/reformatted
    
    // Scheduling
    status: crossPostStatusEnum("status").default("pending").notNull(),
    scheduledFor: timestamp("scheduled_for"),
    postedAt: timestamp("posted_at"),
    
    // Platform response
    platformPostId: varchar("platform_post_id"),
    platformPostUrl: varchar("platform_post_url"),
    platformResponse: jsonb("platform_response").default({}),
  ...
  (table) => [
    index("idx_starz_cross_posts_profile").on(table.profileId),
    index("idx_starz_cross_posts_connection").on(table.connectionId),
    index("idx_starz_cross_posts_content").on(table.contentId),
    index("idx_starz_cross_posts_status").on(table.status),
    index("idx_starz_cross_posts_scheduled").on(table.scheduledFor),
  ],
);

// Platform analytics aggregation
export const starzPlatformAnalytics = pgTable(
  "starz_platform_analytics",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    ...
    engagements: bigint("engagements", { mode: "number" }).default(0),
  ...
  (table) => [
    index("idx_starz_platform_analytics_connection").on(table.connectionId),
    index("idx_starz_platform_analytics_profile").on(table.profileId),
    index("idx_starz_platform_analytics_period").on(table.periodStart, table.periodEnd),
  ],
);

// Content import queue from other platforms (migration tool)
...
export const starzContentImports = pgTable(
  "starz_content_imports",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    profileId: varchar("profile_id")
      ...
      .references(() => profiles.id, { onDelete: "cascade" }),
    connectionId: varchar("connection_id")
      .references(() => starzPlatformConnections.id),
    
    // Source details
    sourcePlatform: auxiliaryPlatformEnum("source_platform").notNull(),
    sourceContentId: varchar("source_content_id"),
    sourceUrl: text("source_url"),
    
    // Import status
    status: contentImportStatusEnum("status").default("pending").notNull(),
    
    // Imported content reference
    importedContentId: varchar("imported_content_id")
      .references(() => content.id),
    importedMediaIds: text("imported_media_ids").array().default([]),
    
    // Processing details
    originalMetadata: jsonb("original_metadata").default({}),
    processedMetadata: jsonb("processed_metadata").default({}),
    conversionLog: jsonb("conversion_log").default([]),
  ...
  (table) => [
    index("idx_starz_content_imports_profile").on(table.profileId),
    index("idx_starz_content_imports_status").on(table.status),
    index("idx_starz_content_imports_source").on(table.sourcePlatform),
  ],
);

// Platform capability definitions
export const auxiliaryPlatformCapabilities = pgTable(
  "auxiliary_platform_capabilities",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    platform: auxiliaryPlatformEnum("platform").notNull().unique(),
    
    // Display info
    displayName: varchar("display_name").notNull(),
    description: text("description"),
    iconUrl: varchar("icon_url"),
    color: varchar("color"), // Brand color
    ...
    supportsAutoPost: boolean("supports_auto_post").default(false),
    supportsAnalytics: boolean("supports_analytics").default(false),
    supportsContentImport: boolean("supports_content_import").default(false),
    supportsScheduling: boolean("supports_scheduling").default(false),
    supportsDirectMessages: boolean("supports_direct_messages").default(false),
    
    // Content type support
    supportsImages: boolean("supports_images").default(true),
    supportsVideos: boolean("supports_videos").default(true),
    supportsText: boolean("supports_text").default(true),
    supportsLinks: boolean("supports_links").default(true),
    
    // Limits
    maxMediaPerPost: integer("max_media_per_post").default(4),
    maxCaptionLength: integer("max_caption_length").default(2200),
    maxHashtags: integer("max_hashtags").default(30),
    maxVideoLength: integer("max_video_length"), // seconds
  ...
  (table) => [
    index("idx_auxiliary_platform_capabilities_platform").on(table.platform),
    index("idx_auxiliary_platform_capabilities_tier").on(table.minimumTier),
  ],
);


// ===== AI HUB INTEGRATION (ai.fanz.website) =====
// Central AI services hub for all FANZ platforms and Starz Studio

export const aiHubServiceStatusEnum = pgEnum("ai_hub_service_status", [
  "available",
  "degraded",
  "maintenance",
  "offline",
...
// AI Hub service definitions
export const aiHubServices = pgTable(
  "ai_hub_services",
  {
    id: varchar("id")
      .primaryKey()
    ...
    name: varchar("name").notNull(),
    description: text("description"),
    category: varchar("category").notNull(), // content, moderation, analytics, automation, assistant
    
    // AI Hub endpoint
    aiHubEndpoint: varchar("ai_hub_endpoint").notNull(), // Path on ai.fanz.website
    httpMethod: varchar("http_method").default("POST"),
    
    // AI model info
    aiModel: varchar("ai_model"), // gpt-4o, claude-3-opus, custom
    modelVersion: varchar("model_version"),
    
    // Status
    status: aiHubServiceStatusEnum("status").default("available").notNull(),
    statusMessage: text("status_message"),
    lastHealthCheck: timestamp("last_health_check"),
    
    // Tier and usage
    minimumTier: starzTierEnum("minimum_tier").default("bronze_star"),
    usageLimits: jsonb("usage_limits").default({}), // Per tier limits
    rateLimit: integer("rate_limit").default(100), // Requests per hour
    
    // Pricing (for metered services)
    pricePerRequest: decimal("price_per_request", { precision: 10, scale: 6 }),
    includedRequests: integer("included_requests").default(0), // Free requests per period
    
    // Configuration
    inputSchema: jsonb("input_schema").default({}), // Expected input format
    outputSchema: jsonb("output_schema").default({}), // Expected output format
  ...
  (table) => [
    index("idx_ai_hub_services_slug").on(table.slug),
    index("idx_ai_hub_services_category").on(table.category),
    index("idx_ai_hub_services_tier").on(table.minimumTier),
  ],
);

// AI Hub usage tracking
export const aiHubUsage = pgTable(
  "ai_hub_usage",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    profileId: varchar("profile_id")
    ...
    serviceId: varchar("service_id")
      .notNull()
      .references(() => aiHubServices.id, { onDelete: "cascade" }),
    membershipId: varchar("membership_id")
      .references(() => starzMemberships.id),
    ...
    wasBillable: boolean("was_billable").default(false),
    
    // Context
    clientPlatform: varchar("client_platform").default("boyfanz"), // Which FANZ platform
    ipAddress: inet("ip_address"),
    userAgent: text("user_agent"),
  ...
  (table) => [
    index("idx_ai_hub_usage_profile").on(table.profileId),
    index("idx_ai_hub_usage_service").on(table.serviceId),
    index("idx_ai_hub_usage_request_id").on(table.requestId),
    index("idx_ai_hub_usage_requested_at").on(table.requestedAt),
  ],
);

// AI Hub quotas per membership
export const aiHubQuotas = pgTable(
  "ai_hub_quotas",
  {
    id: varchar("id")
      .primaryKey()
    ...
    overageUsed: integer("overage_used").default(0),
  ...
  (table) => [
    unique().on(table.membershipId, table.serviceId, table.periodStart),
    index("idx_ai_hub_quotas_membership").on(table.membershipId),
    index("idx_ai_hub_quotas_service").on(table.serviceId),
    index("idx_ai_hub_quotas_period").on(table.periodStart, table.periodEnd),
  ],
);


// ===== FANZ MEDIA HUB INTEGRATION =====
// Central media processing service (fanzmediahub) handles:
// - Forensic signatures, watermarking
// - Media quality scoring for Starz qualification
// - Content fingerprinting

export const mediaHubProcessingStatusEnum = pgEnum("media_hub_processing_status", [
  "queued",
  "processing",
  "completed",
  "failed",
...
// Media Hub processing jobs
export const mediaHubJobs = pgTable(
  "media_hub_jobs",
  {
    id: varchar("id")
      .primaryKey()
    ...
    // Processing type
    jobType: varchar("job_type").notNull(), // quality_score, forensic_signature, watermark, transcode
    
    // Status
    status: mediaHubProcessingStatusEnum("status").default("queued").notNull(),
    progress: integer("progress").default(0), // 0-100
    
    // MediaHub job reference
    mediaHubJobId: varchar("media_hub_job_id"),
    mediaHubCallbackUrl: varchar("media_hub_callback_url"),
    
    // Results (populated by webhook from FanzMediaHub)
    qualityScoreResult: jsonb("quality_score_result").default({}),
    forensicSignature: text("forensic_signature"),
  ...
  (table) => [
    index("idx_media_hub_jobs_media").on(table.mediaAssetId),
    index("idx_media_hub_jobs_profile").on(table.profileId),
    index("idx_media_hub_jobs_status").on(table.status),
    index("idx_media_hub_jobs_type").on(table.jobType),
    index("idx_media_hub_jobs_media_hub_id").on(table.mediaHubJobId),
  ],
);

// Webhook events from FanzMediaHub
export const mediaHubWebhookEvents = pgTable(
  "media_hub_webhook_events",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    jobId: varchar("job_id")
      .references(() => mediaHubJobs.id, { onDelete: "set null" }),
    
    // Event details
    eventType: varchar("event_type").notNull(), // job.completed, job.failed, quality.scored
  ...
  (table) => [
    index("idx_media_hub_webhook_events_job").on(table.jobId),
    index("idx_media_hub_webhook_events_type").on(table.eventType),
    index("idx_media_hub_webhook_events_processed").on(table.processed),
  ],
);


// ===== FANZ HUB VAULT INTEGRATION =====
// Central user status/tier management service (fanzhubvault)
// Handles: User positions, titles, Starz tier upgrades, achievements

// HubVault sync status for user tier changes
export const hubVaultSyncStatusEnum = pgEnum("hub_vault_sync_status", [
  "pending",
  "synced",
  "failed",
  "conflict",
...
// HubVault tier sync records
export const hubVaultTierSync = pgTable(
  "hub_vault_tier_sync",
  {
    id: varchar("id")
      .primaryKey()
    ...
    // Sync direction
    syncDirection: varchar("sync_direction").notNull(), // to_vault, from_vault
    ...
    // HubVault reference
    vaultRecordId: varchar("vault_record_id"),
    vaultUserId: varchar("vault_user_id"),
    
    // Status
    status: hubVaultSyncStatusEnum("status").default("pending").notNull(),
  ...
  (table) => [
    index("idx_hub_vault_tier_sync_profile").on(table.profileId),
    index("idx_hub_vault_tier_sync_status").on(table.status),
    index("idx_hub_vault_tier_sync_vault_user").on(table.vaultUserId),
  ],
);

// HubVault achievement definitions (synced from FanzHubVault)
export const hubVaultAchievements = pgTable(
  "hub_vault_achievements",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    vaultAchievementId: varchar("vault_achievement_id").unique().notNull(),
    
    // Achievement details
    slug: varchar("slug").unique().notNull(),
    name: varchar("name").notNull(),
    description: text("description"),
    category: varchar("category").notNull(), // starz, creator, fan, community
    
    // Requirements
    requirements: jsonb("requirements").default({}),
    
    // Rewards
    rewards: jsonb("rewards").default({}), // XP, badges, perks
    tierPoints: integer("tier_points").default(0), // Contributes to Starz tier
  ...
  (table) => [
    index("idx_hub_vault_achievements_slug").on(table.slug),
    index("idx_hub_vault_achievements_category").on(table.category),
  ],
);


// ===== FANZDASH STARZ VIP INTEGRATION =====
// Admin dashboard integration for Starz member VIP treatment

// Starz member VIP settings for FanzDash
export const starzVipSettings = pgTable(
  "starz_vip_settings",
  {
    id: varchar("id")
      .primaryKey()
    ...
    // VIP display settings
    showBadge: boolean("show_badge").default(true),
    badgePosition: varchar("badge_position").default("top-right"), // top-right, bottom-right, overlay
    customBadgeUrl: varchar("custom_badge_url"), // Custom badge image
    
    // VIP title
    customTitle: varchar("custom_title"), // Optional custom title
    showTierLevel: boolean("show_tier_level").default(true),
    
    // FanzDash visibility
    featuredInDashboard: boolean("featured_in_dashboard").default(true),
    dashboardPriority: integer("dashboard_priority").default(0), // Higher = more prominent
    
    // VIP perks
    prioritySupport: boolean("priority_support").default(false),
    dedicatedAccountManager: varchar("dedicated_account_manager"),
    customFeatures: jsonb("custom_features").default({}),
    
    // Analytics preferences
    showInLeaderboards: boolean("show_in_leaderboards").default(true),
    showEarningsPublicly: boolean("show_earnings_publicly").default(false),
    
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
  },
  (table) => [
    index("idx_starz_vip_settings_membership").on(table.membershipId),
    index("idx_starz_vip_settings_profile").on(table.profileId),
    index("idx_starz_vip_settings_featured").on(table.featuredInDashboard),
    index("idx_starz_vip_settings_priority").on(table.dashboardPriority),
  ],
);

// Starz leaderboard tracking
export const starzLeaderboard = pgTable(
  "starz_leaderboard",
  {
    id: varchar("id")
      .primaryKey()
    ...
    tierRank: integer("tier_rank"), // Rank within their tier
    categoryRank: integer("category_rank"), // Rank in their content category
    
    // Metrics for ranking
    totalScore: integer("total_score").default(0),
    fanGrowth: integer("fan_growth").default(0),
    engagementScore: integer("engagement_score").default(0),
    qualityScore: integer("quality_score").default(0),
    referralScore: integer("referral_score").default(0),
    
    // Snapshot of tier at ranking time
    tierAtRanking: starzTierEnum("tier_at_ranking"),
    
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
  },
  (table) => [
    unique().on(table.profileId, table.periodType, table.periodStart),
    index("idx_starz_leaderboard_profile").on(table.profileId),
    index("idx_starz_leaderboard_period").on(table.periodType, table.periodStart),
    index("idx_starz_leaderboard_rank").on(table.overallRank),
    index("idx_starz_leaderboard_tier_rank").on(table.tierRank),
  ],
);


// ===== FANZDASH EMAIL & COMMUNICATION SYSTEM =====
// Admin-initiated communications to Starz members and users
...
// Email templates for FanzDash
export const fanzdashEmailTemplates = pgTable(
  "fanzdash_email_templates",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    ...
    // Targeting
    targetTiers: text("target_tiers").array().default([]), // Which Starz tiers
    targetConditions: jsonb("target_conditions").default({}), // Additional targeting
    ...
    createdBy: varchar("created_by").references(() => accounts.id),
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
  },
  (table) => [
    index("idx_fanzdash_email_templates_type").on(table.type),
    index("idx_fanzdash_email_templates_slug").on(table.slug),
  ],
);

// Email campaigns from FanzDash
export const fanzdashEmailCampaigns = pgTable(
  "fanzdash_email_campaigns",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    ...
    description: text("description"),
    templateId: varchar("template_id")
      .references(() => fanzdashEmailTemplates.id),
    
    // Targeting
    targetAudience: varchar("target_audience").notNull(), // all, starz_only, tier_specific, custom
    targetTiers: text("target_tiers").array().default([]),
    targetProfileIds: text("target_profile_ids").array().default([]), // Specific profiles
    targetConditions: jsonb("target_conditions").default({}),
    
    // Content (can override template)
    customSubject: varchar("custom_subject"),
    customHtmlBody: text("custom_html_body"),
    customTextBody: text("custom_text_body"),
  ...
  (table) => [
    index("idx_fanzdash_email_campaigns_status").on(table.status),
    index("idx_fanzdash_email_campaigns_scheduled").on(table.scheduledFor),
    index("idx_fanzdash_email_campaigns_created_by").on(table.createdBy),
  ],
);

// Individual email sends
export const fanzdashEmailSends = pgTable(
  "fanzdash_email_sends",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    campaignId: varchar("campaign_id")
      .notNull()
      .references(() => fanzdashEmailCampaigns.id, { onDelete: "cascade" }),
    profileId: varchar("profile_id")
      .notNull()
      .references(() => profiles.id, { onDelete: "cascade" }),
  ...
  (table) => [
    index("idx_fanzdash_email_sends_campaign").on(table.campaignId),
    index("idx_fanzdash_email_sends_profile").on(table.profileId),
    index("idx_fanzdash_email_sends_status").on(table.status),
    index("idx_fanzdash_email_sends_message_id").on(table.messageId),
  ],
);

// Quick thank you messages (admin can send directly)
export const starzThankYouMessages = pgTable(
  "starz_thank_you_messages",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    fromAdminId: varchar("from_admin_id")
      .notNull()
      .references(() => accounts.id),
    toProfileId: varchar("to_profile_id")
      .notNull()
      .references(() => profiles.id, { onDelete: "cascade" }),
    toMembershipId: varchar("to_membership_id")
      .references(() => starzMemberships.id),
    ...
    // Type
    messageType: varchar("message_type").default("thank_you"), // thank_you, congratulations, milestone, custom
    triggerReason: varchar("trigger_reason"), // tier_upgrade, achievement, manual
    
    // Delivery
    sentViaEmail: boolean("sent_via_email").default(true),
    sentViaNotification: boolean("sent_via_notification").default(true),
  ...
  (table) => [
    index("idx_starz_thank_you_from_admin").on(table.fromAdminId),
    index("idx_starz_thank_you_to_profile").on(table.toProfileId),
    index("idx_starz_thank_you_type").on(table.messageType),
  ],
);


// =============================================================================
// CREATOR PROMOTIONS & COUPONS
// =============================================================================
...
export const creatorPromotions = pgTable("creator_promotions", {
  ...
  bundleMonths: integer("bundle_months"),
  minPurchaseMonths: integer("min_purchase_months").default(1),
  ...
  isPublic: boolean("is_public").default(true),
  newSubscribersOnly: boolean("new_subscribers_only").default(false),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const creatorCoupons = pgTable("creator_coupons", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  creatorId: varchar("creator_id").notNull(),
  ...
  expiresAt: timestamp("expires_at"),
  status: couponStatus("status").default("active").notNull(),
  newSubscribersOnly: boolean("new_subscribers_only").default(false),
  requiresInfluencerCode: varchar("requires_influencer_code"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const discountAnalytics = pgTable("discount_analytics", {
  ...
  totalPromotionsActive: integer("total_promotions_active").default(0),
  totalCouponsActive: integer("total_coupons_active").default(0),
  promotionRedemptions: integer("promotion_redemptions").default(0),
  couponRedemptions: integer("coupon_redemptions").default(0),
  revenueFromDiscountedSubs: integer("revenue_from_discounted_subs").default(0),
  revenueLostToDiscounts: integer("revenue_lost_to_discounts").default(0),
  newSubsFromPromos: integer("new_subs_from_promos").default(0),
  retentionRate: decimal("retention_rate", { precision: 5, scale: 2 }),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

// ===== AI CONTROL PLANE SCHEMA =====
// Agent Registry, RBAC, Policy, and Audit system for FANZ OS autonomy agents

export const aiAgentTier = pgEnum("ai_agent_tier", ["internal", "creator_visible"]);
export const aiAgentStatus = pgEnum("ai_agent_status", ["draft", "active", "disabled"]);
export const aiAuditActorType = pgEnum("ai_audit_actor_type", ["agent", "human", "system"]);
export const aiAuditResult = pgEnum("ai_audit_result", ["allow", "deny", "info"]);
export const aiPolicyTier = pgEnum("ai_policy_tier", ["core", "persona", "high_risk"]);

// AI Agents - main registry for all AI/autonomy agents
export const aiAgents = pgTable("ai_agents", {
  id: varchar("id").primaryKey(), // e.g., "fanzautoheal", "fanzoptimizer"
  name: varchar("name", { length: 100 }).notNull(),
  description: text("description"),
  tier: aiAgentTier("tier").notNull().default("internal"),
  domain: varchar("domain", { length: 50 }).notNull(), // ops_autonomy, performance_cost, database, etc.
  status: aiAgentStatus("status").notNull().default("draft"),
  purpose: text("purpose"),
  systemPrompt: text("system_prompt"),
  allowedActions: jsonb("allowed_actions").default([]), // string[]
  deniedActions: jsonb("denied_actions").default([]), // string[]
  inputs: jsonb("inputs").default([]), // string[] - what data this agent can read
  outputs: jsonb("outputs").default([]), // string[] - what this agent produces
  platformScopes: jsonb("platform_scopes").default([]), // which FANZ platforms it can operate on
  dataScopes: jsonb("data_scopes").default([]), // what data it can access
  effectScopes: jsonb("effect_scopes").default([]), // what it can write/trigger
  constraints: jsonb("constraints").default({}), // agent-specific constraints
  version: varchar("version", { length: 20 }).default("1.0.0"),
  ownerId: varchar("owner_id"), // team/user who owns this agent
  riskProfile: varchar("risk_profile", { length: 20 }).default("low"), // low/medium/high
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

// AI Agent Versions - version history for agents
export const aiAgentVersions = pgTable("ai_agent_versions", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  agentId: varchar("agent_id").notNull(),
  version: varchar("version", { length: 20 }).notNull(),
  snapshot: jsonb("snapshot").notNull(), // full agent config at this version
  changeNote: text("change_note"),
  changedBy: varchar("changed_by"),
  promotedToProd: boolean("promoted_to_prod").default(false),
  promotedAt: timestamp("promoted_at"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

// AI Roles - RBAC roles for agents
export const aiRoles = pgTable("ai_roles", {
  id: varchar("id").primaryKey(),
  name: varchar("name", { length: 100 }).notNull(),
  description: text("description"),
  permissions: jsonb("permissions").default([]), // string[] - resource:action format
  constraints: jsonb("constraints").default({}), // role-level constraints
  environment: varchar("environment", { length: 20 }).default("prod"), // dev/staging/prod
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

// AI Agent Role Bindings - many-to-many between agents and roles
export const aiAgentRoleBindings = pgTable("ai_agent_role_bindings", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  agentId: varchar("agent_id").notNull(),
  roleId: varchar("role_id").notNull(),
  expiresAt: timestamp("expires_at"), // time-boxed grants
  grantedBy: varchar("granted_by"),
  grantedAt: timestamp("granted_at").defaultNow().notNull(),
});

// AI Audit Events - immutable audit trail for all agent actions
export const aiAuditEvents = pgTable("ai_audit_events", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  time: timestamp("time").defaultNow().notNull(),
  actorType: aiAuditActorType("actor_type").notNull(),
  actorId: varchar("actor_id").notNull(),
  action: varchar("action", { length: 100 }).notNull(),
  resourceType: varchar("resource_type", { length: 100 }),
  resourceId: varchar("resource_id"),
  platform: varchar("platform", { length: 50 }), // boyfanz, girlfanz, etc.
  result: aiAuditResult("result").notNull(),
  decisionId: varchar("decision_id"),
  correlationId: varchar("correlation_id"),
  traceParent: varchar("trace_parent"),
  payloadRedacted: jsonb("payload_redacted").default({}), // PII-redacted request/response
  matchedRules: jsonb("matched_rules").default([]),
  violatedConstraints: jsonb("violated_constraints").default([]),
});

// AI Policy Decisions - record of policy evaluations
export const aiPolicyDecisions = pgTable("ai_policy_decisions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull(),
  action: varchar("action", { length: 100 }).notNull(),
  resource: varchar("resource", { length: 200 }).notNull(),
  platform: varchar("platform", { length: 50 }),
  tier: aiPolicyTier("tier").notNull(),
  subjectId: varchar("subject_id"), // creator_id/content_id/txn_id
  context: jsonb("context").default({}),
  allow: boolean("allow").notNull(),
  matchedRules: jsonb("matched_rules").default([]),
  violatedConstraints: jsonb("violated_constraints").default([]),
  obligations: jsonb("obligations").default([]), // required audit events
  effectivePermissions: jsonb("effective_permissions").default([]),
  evaluatedAt: timestamp("evaluated_at").defaultNow().notNull(),
});

// AI Approval Tokens - for human-in-the-loop approval workflow
export const aiApprovalTokens = pgTable("ai_approval_tokens", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  proposalId: varchar("proposal_id").notNull(),
  agentId: varchar("agent_id").notNull(),
  action: varchar("action", { length: 100 }).notNull(),
  scope: jsonb("scope").notNull(), // what this token authorizes
  riskLevel: varchar("risk_level", { length: 20 }).notNull(),
  rollbackPlan: text("rollback_plan"),
  canaryPlan: text("canary_plan"),
  requiredSigners: jsonb("required_signers").default([]), // who needs to approve
  signoffs: jsonb("signoffs").default([]), // collected approvals
  status: varchar("status", { length: 20 }).default("pending"), // pending/approved/rejected/expired/revoked
...
// Insert schemas for AI Control Plane
export const insertAiAgentSchema = createInsertSchema(aiAgents);
export const insertAiRoleSchema = createInsertSchema(aiRoles);
export const insertAiAgentRoleBindingSchema = createInsertSchema(aiAgentRoleBindings);
export const insertAiAuditEventSchema = createInsertSchema(aiAuditEvents);
export const insertAiPolicyDecisionSchema = createInsertSchema(aiPolicyDecisions);
export const insertAiApprovalTokenSchema = createInsertSchema(aiApprovalTokens);

// ===== FANZ CYBERSECURITY SYSTEM SCHEMA =====
...
export const securityBotCategoryEnum = pgEnum("security_bot_category", [
  "network",
  "auth",
  "app",
  "data",
...
// Security Events - Central event log for all 500 bots
export const securityEvents = pgTable("security_events", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  eventType: securityEventTypeEnum("event_type").notNull(),
  severity: securitySeverityEnum("severity").notNull(),
  botId: varchar("bot_id", { length: 20 }).notNull(), // SEC-001 through SEC-500
  botCategory: securityBotCategoryEnum("bot_category").notNull(),
  platform: varchar("platform", { length: 50 }), // boyfanz, girlfanz, etc.
  sourceIp: inet("source_ip"),
  targetResource: varchar("target_resource", { length: 500 }),
  userId: varchar("user_id"),
  sessionId: varchar("session_id"),
  requestPath: varchar("request_path", { length: 1000 }),
  ...
  index("idx_security_events_bot").on(table.botId),
  index("idx_security_events_platform").on(table.platform),
  index("idx_security_events_ip").on(table.sourceIp),
  index("idx_security_events_user").on(table.userId),
  index("idx_security_events_created").on(table.createdAt),
]);

// Blocked IPs - IP blocking and rate limiting
export const blockedIps = pgTable("blocked_ips", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ipAddress: inet("ip_address").notNull(),
  reason: varchar("reason", { length: 500 }).notNull(),
  blockedBy: varchar("blocked_by", { length: 20 }).notNull(), // Bot ID or admin
  blockType: varchar("block_type", { length: 50 }).notNull(), // permanent, temporary, rate_limit
  severity: securitySeverityEnum("severity").notNull(),
  attackCount: integer("attack_count").default(1),
  ...
  index("idx_blocked_ips_address").on(table.ipAddress),
  index("idx_blocked_ips_expires").on(table.expiresAt),
]);

// Security Bot Status - Track all 500 bots
export const securityBotStatus = pgTable("security_bot_status", {
  botId: varchar("bot_id", { length: 20 }).primaryKey(), // SEC-001 through SEC-500
  name: varchar("name", { length: 200 }).notNull(),
  category: securityBotCategoryEnum("category").notNull(),
  description: text("description"),
  isRunning: boolean("is_running").default(false),
  lastCheck: timestamp("last_check"),
  lastAlert: timestamp("last_alert"),
  checksPerformed: bigint("checks_performed", { mode: "number" }).default(0),
  threatsDetected: bigint("threats_detected", { mode: "number" }).default(0),
  threatsBlocked: bigint("threats_blocked", { mode: "number" }).default(0),
  falsePositives: bigint("false_positives", { mode: "number" }).default(0),
  uptime: bigint("uptime", { mode: "number" }).default(0), // seconds
  ...
  index("idx_security_bot_category").on(table.category),
  index("idx_security_bot_running").on(table.isRunning),
]);

// Attack Patterns - ML pattern detection
export const attackPatterns = pgTable("attack_patterns", {
  ...
  signature: text("signature").notNull(), // regex or signature
  severity: securitySeverityEnum("severity").notNull(),
  category: securityBotCategoryEnum("category").notNull(),
  description: text("description"),
  ...
  detectionCount: bigint("detection_count", { mode: "number" }).default(0),
  lastDetectedAt: timestamp("last_detected_at"),
  falsePositiveRate: decimal("false_positive_rate", { precision: 5, scale: 4 }).default("0"),
  confidence: integer("confidence").default(80), // 0-100
  isActive: boolean("is_active").default(true),
  createdBy: varchar("created_by", { length: 20 }), // Bot that discovered it
  metadata: jsonb("metadata").default({}),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => [
  index("idx_attack_patterns_type").on(table.patternType),
  index("idx_attack_patterns_category").on(table.category),
  index("idx_attack_patterns_active").on(table.isActive),
]);

// Security Alerts - Aggregated alerts for admin
export const securityAlerts = pgTable("security_alerts", {
  ...
  severity: securitySeverityEnum("severity").notNull(),
  category: securityBotCategoryEnum("category").notNull(),
  botIds: jsonb("bot_ids").default([]), // Bots that triggered this alert
  ...
  status: varchar("status", { length: 50 }).default("open"), // open, acknowledged, resolved, false_positive
  ...
  index("idx_security_alerts_severity").on(table.severity),
  index("idx_security_alerts_status").on(table.status),
  index("idx_security_alerts_created").on(table.createdAt),
]);

// Rate Limit Rules - Configurable rate limiting
export const rateLimitRules = pgTable("rate_limit_rules", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  name: varchar("name", { length: 200 }).notNull(),
  description: text("description"),
  endpoint: varchar("endpoint", { length: 500 }), // null = global
  ...
  byIp: boolean("by_ip").default(true),
  byUser: boolean("by_user").default(false),
  bySession: boolean("by_session").default(false),
  exemptRoles: jsonb("exempt_roles").default([]),
  exemptIps: jsonb("exempt_ips").default([]),
  isActive: boolean("is_active").default(true),
  priority: integer("priority").default(0),
  createdBy: varchar("created_by"),
  metadata: jsonb("metadata").default({}),
  ...
  index("idx_rate_limit_rules_endpoint").on(table.endpoint),
  index("idx_rate_limit_rules_active").on(table.isActive),
]);

// BOT LAW Violations - Track compliance violations
export const botLawViolations = pgTable("bot_law_violations", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull(),
  ...
  correctedConfig: jsonb("corrected_config").default({}),
  ...
  index("idx_bot_law_violations_agent").on(table.agentId),
  index("idx_bot_law_violations_type").on(table.violationType),
  index("idx_bot_law_violations_severity").on(table.severity),
]);

// Threat Intelligence - External threat feeds
export const threatIntelligence = pgTable("threat_intelligence", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  indicator: varchar("indicator", { length: 500 }).notNull(), // IP, domain, hash, etc
  indicatorType: varchar("indicator_type", { length: 50 }).notNull(), // ip, domain, url, hash, email
  threatType: varchar("threat_type", { length: 100 }).notNull(), // malware, phishing, botnet, etc
  severity: securitySeverityEnum("severity").notNull(),
  confidence: integer("confidence").default(80), // 0-100
  source: varchar("source", { length: 200 }).notNull(), // Feed name
  tags: jsonb("tags").default([]),
  ...
  index("idx_threat_intel_indicator").on(table.indicator),
  index("idx_threat_intel_type").on(table.indicatorType),
  index("idx_threat_intel_active").on(table.isActive),
]);

// Security Metrics - Time-series metrics
export const securityMetrics = pgTable("security_metrics", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  metricName: varchar("metric_name", { length: 200 }).notNull(),
  botId: varchar("bot_id", { length: 20 }),
  category: securityBotCategoryEnum("category"),
  platform: varchar("platform", { length: 50 }),
  ...
  index("idx_security_metrics_name").on(table.metricName),
  index("idx_security_metrics_bot").on(table.botId),
  index("idx_security_metrics_timestamp").on(table.timestamp),
]);

// AI Capability Toggles - FANZ BOT LAW capability system
export const aiCapabilityToggles = pgTable("ai_capability_toggles", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull(),
  ...
  requiresTraining: boolean("requires_training").default(true),
  ...
  unique("unique_agent_capability").on(table.agentId, table.capability),
  index("idx_capability_toggles_agent").on(table.agentId),
  index("idx_capability_toggles_capability").on(table.capability),
]);

// Insert schemas for Security System
...
export const insertSecurityBotStatusSchema = createInsertSchema(securityBotStatus);
export const insertAttackPatternSchema = createInsertSchema(attackPatterns);
...
export const insertBotLawViolationSchema = createInsertSchema(botLawViolations);
...
export const insertAiCapabilityToggleSchema = createInsertSchema(aiCapabilityToggles);
...
export type SecurityBotStatus = typeof securityBotStatus.$inferSelect;
export type InsertSecurityBotStatus = typeof securityBotStatus.$inferInsert;
export type AttackPattern = typeof attackPatterns.$inferSelect;
export type InsertAttackPattern = typeof attackPatterns.$inferInsert;
...
export type BotLawViolation = typeof botLawViolations.$inferSelect;
export type InsertBotLawViolation = typeof botLawViolations.$inferInsert;
...
export type AiCapabilityToggle = typeof aiCapabilityToggles.$inferSelect;
export type InsertAiCapabilityToggle = typeof aiCapabilityToggles.$inferInsert;
