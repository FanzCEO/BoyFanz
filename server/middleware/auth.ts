...
/**
 * Check if user should bypass charges (super admin, admin, or moderator)
 */
...
// Middleware to check if user is authenticated (supports both Passport and FanzSSO)
export function isAuthenticated(req: Request, res: Response, next: NextFunction) {
  // Check FanzSSO authentication first
...
// Middleware to check if user is admin (supports both Passport and FanzSSO)
export function requireAdmin(req: Request, res: Response, next: NextFunction) {
  // Check FanzSSO authentication first
  const ssoUser = req.ssoUser || req.session?.ssoUser;
  if (ssoUser) {
...
// Middleware to check if user is creator only (supports FanzSSO)
export function requireCreator(req: Request, res: Response, next: NextFunction) {
  // Check FanzSSO authentication first
  const ssoUser = req.ssoUser || req.session?.ssoUser;
  if (ssoUser) {
    if (isSuperAdmin(ssoUser.email) || ssoUser.isAdmin || ssoUser.isCreator || ssoUser.roles?.includes('creator')) {
      return next();
    }
  ...
  if (isSuperAdmin(req.user?.email) || req.user?.role === 'admin' || req.user?.role === 'creator') {
    return next();
  }
...
// Middleware to check if user is creator or admin (supports FanzSSO)
export function requireCreatorOrAdmin(req: Request, res: Response, next: NextFunction) {
  // Check FanzSSO authentication first
  const ssoUser = req.ssoUser || req.session?.ssoUser;
  if (ssoUser) {
    if (isSuperAdmin(ssoUser.email) || ssoUser.isAdmin || ssoUser.isCreator) {
      return next();
    }
  ...
  if (isSuperAdmin(req.user?.email) || req.user?.role === 'creator' || req.user?.role === 'admin') {
    return next();
  }
...
// Middleware to check if user is moderator or admin (supports FanzSSO)
export function requireModeratorOrAdmin(req: Request, res: Response, next: NextFunction) {
  // Check FanzSSO authentication first
  const ssoUser = req.ssoUser || req.session?.ssoUser;
  if (ssoUser) {
    if (isSuperAdmin(ssoUser.email) || ssoUser.isAdmin || ssoUser.isModerator) {
      return next();
    }
  ...
  if (isSuperAdmin(req.user?.email) || req.user?.role === 'moderator' || req.user?.role === 'admin') {
    return next();
  }
