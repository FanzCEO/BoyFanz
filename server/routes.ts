...
import { creatorPromotions, creatorCoupons, couponRedemptions, promotionRedemptions, identityVerifications, profiles } from "@shared/schema";
...
import appearanceRoutes from './routes/admin/appearance';
import galleryRoutes from './routes/admin/gallery';
// Section 7: Admin Panel & Analytics Routes
import analyticsRoutes from './routes/admin/analyticsRoutes';
import fraudRoutes from './routes/admin/fraudRoutes';
import creatorHealthRoutes from './routes/admin/creatorHealthRoutes';
import experimentsRoutes from './routes/admin/experimentsRoutes';
import supportTicketsRoutes from './routes/admin/supportTicketsRoutes';
import auditRoutes from './routes/admin/auditRoutes';
import creatorAnalyticsRoutes from './routes/admin/creatorAnalyticsRoutes';
...
export function registerRoutes(app: Express) {
  ...
  app.use("/api/fuck-buddies", fuckBuddyRoutes);
  app.use("/api/naughty-profile", naughtyProfileRoutes);

  // ===== TIP GAMES & GAMIFICATION =====
  // Wheel of Fortune, Slots, Dice, Mystery Box games
  app.use("/api/tip-games", tipGamesRoutes);

  // ===== VIP TIERS SYSTEM =====
  // Bronze, Silver, Gold, Diamond subscription tiers
  app.use("/api/vip-tiers", vipTiersRoutes);

  // ===== XP, BADGES & LEADERBOARDS =====
  // Gamification system - XP levels, achievement badges, leaderboards
  ...
  // ===== CREATOR BATTLES =====
  // Head-to-head creator competitions
  app.use("/api/battles", battlesRoutes);

  // ===== AI CLONE CHATBOTS =====
  // AI-powered creator clones for 24/7 fan engagement
  app.use("/api/ai-clone", aiCloneRoutes);

  // ===== AUCTIONS & TIP GOALS =====
  // Live auction house and crowdfunding goals
  app.use("/api/auction", auctionRoutes);

  // ===== WATCH PARTIES & COLLABS =====
  // Group viewing and creator collaboration management
  ...
  // ===== AGE VERIFICATION =====
  // VerifyMy integration for age verification (fans & creators)
  ...
  app.use("/api/admin/appearance", appearanceRoutes);
  app.use("/api/admin/gallery", galleryRoutes);

  // Section 7: Admin Panel & Analytics Routes
  app.use("/api/admin/analytics", analyticsRoutes);
  app.use("/api/admin/fraud", fraudRoutes);
  app.use("/api/admin/creator-health", creatorHealthRoutes);
  app.use("/api/admin/experiments", experimentsRoutes);
  app.use("/api/admin/support-tickets", supportTicketsRoutes);
  ...
  app.get("/api/admin/delegated-permissions/my", isAuthenticated, async (req, res) => {
    try {
      ...
      console.error("Failed to get delegated permissions:", error);
      res.json([]);
    }
  });
  ...
  // ===== 2257 VERIFICATION ROUTES (Form + VerifyMy) =====

  // Get 2257 verification status

  // General verification status endpoint
  app.get("/api/verification/status", isAuthenticated, csrfProtection, async (req, res) => {
    try {
      const userId = req.user!.id;
      
      // Check identity verification
      const identityRecord = await db.execute(sql`SELECT * FROM user_verifications WHERE user_id = ${userId} AND verification_type = 'identity' LIMIT 1`).then(r => r.rows?.[0]);
      
      // Check age verification  
      const ageRecord = await db.execute(sql`SELECT * FROM user_verifications WHERE user_id = ${userId} AND verification_type = 'age' LIMIT 1`).then(r => r.rows?.[0]);
      
      // Check 2257 form
      const form2257 = await db.execute(sql`SELECT * FROM user_verifications WHERE user_id = ${userId} AND verification_type = '2257_form' LIMIT 1`).then(r => r.rows?.[0]);
      
      res.json({
        identityVerified: identityRecord?.status === "verified",
        ageVerified: ageRecord?.status === "verified",
        form2257Submitted: !!form2257,
        form2257Status: form2257?.status || null,
        overallStatus: identityRecord?.status === "verified" && form2257 ? "complete" : "incomplete"
      });
    } catch (error) {
  ...
  app.get("/api/verification/2257/status", isAuthenticated, csrfProtection, async (req, res) => {
    try {
      const userId = req.user!.id;
      
      // Check if user has submitted 2257 form
      const formRecord = await db.query.userVerifications?.findFirst({
        where: and(
          eq(userVerifications.userId, userId),
          eq(userVerifications.verificationType, "2257_form")
        )
      }) || await db.execute(sql`SELECT * FROM user_verifications WHERE user_id = ${userId} AND verification_type = '2257_form' LIMIT 1`).then(r => r.rows?.[0]);
      
      // Check VerifyMy identity verification status
      const identityRecord = await db.query.userVerifications?.findFirst({
        where: and(
          eq(userVerifications.userId, userId),
          eq(userVerifications.verificationType, "identity")
        )
      }) || await db.execute(sql`SELECT * FROM user_verifications WHERE user_id = ${userId} AND verification_type = 'identity' LIMIT 1`).then(r => r.rows?.[0]);
      
      // If no form submitted yet
      if (!formRecord) {
        return res.json({
          status: "not_submitted",
          message: "2257 record not yet submitted",
          formSubmitted: false,
          verifyMyCompleted: false
        });
      }
      
      // Form submitted but VerifyMy not completed
      if (!identityRecord || identityRecord.status !== "verified") {
        return res.json({
          status: "pending_verification",
          message: "Please complete identity verification with VerifyMy",
          formSubmitted: true,
          verifyMyCompleted: false,
          submittedAt: formRecord.created_at || formRecord.createdAt,
      ...
      res.json({
        status: "approved",
        message: "2257 compliance verified",
        formSubmitted: true,
        verifyMyCompleted: true,
        submittedAt: formRecord.created_at || formRecord.createdAt,
        approvedAt: identityRecord.verified_at || identityRecord.verifiedAt,
        recordId: formRecord.id
      });
    } catch (error) {
      console.error("2257 status check error:", error);
      res.status(500).json({ error: "Failed to get 2257 status" });
    }
  });

  // Submit 2257 form - saves data then prompts VerifyMy
  app.post("/api/verification/2257/submit", isAuthenticated, csrfProtection, async (req, res) => {
    try {
      const userId = req.user!.id;
      const {
        legalFirstName,
        ...
        currentAddress,
        attestsOver18,
        attestsIdentityAccurate,
        consentToRecordKeeping
      } = req.body;
      
      // Validate required fields
      if (!legalFirstName || !legalLastName || !dateOfBirth || !primaryIdType || !primaryIdNumber || !primaryIdIssuer || !currentAddress || !idFrontImageUrl || !idBackImageUrl || !selfieWithIdUrl) {
      ...
      if (!attestsOver18 || !attestsIdentityAccurate || !consentToRecordKeeping) {
        return res.status(400).json({ 
          error: "Attestations required",
          message: "You must agree to all attestations to submit" 
        });
      }
      ...
      // Save the 2257 form data to user_verifications
      const fullName = legalMiddleName 
      ...
      const metadata = {
        ...
        primaryId: {
          ...
          issuer: primaryIdIssuer,
          expiryDate: primaryIdExpiryDate || null
        },
        photos: {
          idFrontUrl: idFrontImageUrl,
          idBackUrl: idBackImageUrl,
      ...
      await db.execute(sql`
        INSERT INTO user_verifications (id, user_id, verification_type, status, provider, metadata, created_at, updated_at)
        VALUES (${recordId}, ${userId}, '2257_form', 'pending', 'internal', ${JSON.stringify(metadata)}::jsonb, NOW(), NOW())
      ...
      await storage.createAuditLog({
        userId,
        action: "2257_FORM_SUBMITTED",
        details: JSON.stringify({
          recordId,
          idType: primaryIdType,
          idIssuer: primaryIdIssuer
        }),
        timestamp: new Date()
      });
      ...
      res.json({
        ...
        recordId,
        status: "pending_verification",
        message: "Form saved successfully. Please complete identity verification to finalize.",
        requiresVerifyMy: true,
        verifyMyUrl: "/api/verification/identity/start"
      });
    } catch (error) {
      console.error("2257 submission error:", error);
      res.status(500).json({ error: "Failed to submit 2257 record" });
    }
  });

  // Start VerifyMy identity verification
  app.post("/api/verification/identity/start", isAuthenticated, csrfProtection, async (req, res) => {
    try {
      const userId = req.user!.id;
      const user = req.user!;
      
      // Check if 2257 form was submitted first
      const formRecord = await db.execute(sql`SELECT * FROM user_verifications WHERE user_id = ${userId} AND verification_type = '2257_form' LIMIT 1`).then(r => r.rows?.[0]);
      
      if (!formRecord) {
        return res.status(400).json({
          error: "Form not submitted",
          message: "Please submit the 2257 form first"
        });
      }
      
      // Get VerifyMy config from environment
      const env = process.env.VERIFYMY_ENV || "sandbox";
      const apiKey = process.env.VERIFYMYCONTENT_API_KEY || "";
      const apiSecret = process.env.VERIFYMYCONTENT_API_SECRET || "";
      
      if (!apiKey || !apiSecret) {
        return res.status(500).json({
          error: "VerifyMy not configured",
          message: "Identity verification service is not configured"
        });
      }
      
      const baseUrl = env === "production" 
        ? "https://oauth.verifymycontent.com"
        : "https://oauth.sandbox.verifymycontent.com";
      
      const platformUrl = process.env.PLATFORM_PUBLIC_URL || `https://${req.get('host')}`;
      const webhookBase = process.env.VERIFYMY_WEBHOOK_BASE_URL || platformUrl;
      
      // Build payload
      const formData = formRecord.metadata as any;
      const payload = {
        customer: {
          id: userId,
          email: user.email || `${userId}@placeholder.fanz`,
          first_name: formData?.firstName || "",
          last_name: formData?.lastName || ""
        },
        redirect_uri: `${platformUrl}/verify/verifymy/identity/callback`,
        webhook: `${webhookBase}/api/webhooks/verifymy/identity`
      };
      ...
      // Call VerifyMy API
      const response = await fetch(`${baseUrl}/api/v1/identity-verification`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error("VerifyMy API error:", response.status, errBody);
        return res.status(500).json({
      ...
      const vmResult = await response.json();
      const verificationUrl = vmResult.url || vmResult.redirect_url || vmResult.verification_url;
      const verificationId = vmResult.id || vmResult.identity_verification_id;
      
      // Save/update identity verification record
      await db.execute(sql`
        INSERT INTO user_verifications (id, user_id, verification_type, status, provider, provider_reference, metadata, created_at, updated_at)
        VALUES (${nanoid()}, ${userId}, 'identity', 'pending', 'verifymy', ${verificationId || null}, ${JSON.stringify({ initiated_at: new Date().toISOString() })}::jsonb, NOW(), NOW())
        ON CONFLICT (user_id, verification_type) 
        DO UPDATE SET 
          status = 'pending',
          provider_reference = ${verificationId || null},
          updated_at = NOW()
      `);
      ...
      await storage.createAuditLog({
        userId,
        action: "VERIFYMY_IDENTITY_INITIATED",
        details: JSON.stringify({ verificationId }),
        timestamp: new Date()
      });
      ...
      console.error("VerifyMy initiation error:", error);
...
export async function setupAdvancedRoutes(app: Express) {
  ...
  // =============================================================================
  // CREATOR PROMOTIONS & COUPONS API
  // =============================================================================

  // Get creator's promotions
  app.get('/api/creator/promotions', isAuthenticated, requireCreator, async (req, res) => {
    try {
      const creatorId = (req as any).user?.id || (req as any).ssoUser?.id;
      const promotions = await db.select()
        .from(creatorPromotions)
        .where(eq(creatorPromotions.creatorId, creatorId))
        .orderBy(desc(creatorPromotions.createdAt));
      res.json(promotions);
    } catch (error) {
  ...
  // Create promotion
  app.post('/api/creator/promotions', isAuthenticated, requireCreator, async (req, res) => {
    try {
      const creatorId = (req as any).user?.id || (req as any).ssoUser?.id;
      const { name, description, type, value, trialDays, bundleMonths, minPurchaseMonths, maxRedemptions, startsAt, endsAt, isPublic, newSubscribersOnly } = req.body;
      
      const [promotion] = await db.insert(creatorPromotions).values({
        creatorId,
        name,
        description,
        type,
  ...
  // Update promotion
  app.patch('/api/creator/promotions/:id', isAuthenticated, requireCreator, async (req, res) => {
    try {
      const creatorId = (req as any).user?.id || (req as any).ssoUser?.id;
      const promotionId = parseInt(req.params.id);
      const updates = req.body;
      
      const [promotion] = await db.update(creatorPromotions)
        .set({ ...updates, updatedAt: new Date() })
        .where(and(
          eq(creatorPromotions.id, promotionId),
          eq(creatorPromotions.creatorId, creatorId)
        ))
        .returning();
  ...
  // Delete promotion
  app.delete('/api/creator/promotions/:id', isAuthenticated, requireCreator, async (req, res) => {
    try {
      const creatorId = (req as any).user?.id || (req as any).ssoUser?.id;
      const promotionId = parseInt(req.params.id);
      
      await db.delete(creatorPromotions)
        .where(and(
          eq(creatorPromotions.id, promotionId),
          eq(creatorPromotions.creatorId, creatorId)
        ));
  ...
  app.get('/api/creator/coupons', isAuthenticated, requireCreator, async (req, res) => {
    try {
      const creatorId = (req as any).user?.id || (req as any).ssoUser?.id;
      const coupons = await db.select()
        .from(creatorCoupons)
        .where(eq(creatorCoupons.creatorId, creatorId))
        .orderBy(desc(creatorCoupons.createdAt));
      res.json(coupons);
    } catch (error) {
  ...
  // Create coupon
  app.post('/api/creator/coupons', isAuthenticated, requireCreator, async (req, res) => {
    try {
      const creatorId = (req as any).user?.id || (req as any).ssoUser?.id;
      const { name, description, type, value, trialDays, bundleMonths, minPurchaseMonths, maxRedemptions, maxRedemptionsPerUser, startsAt, expiresAt, newSubscribersOnly, customCode } = req.body;
      
      // Generate or use custom code
      let code = customCode?.toUpperCase().replace(/[^A-Z0-9]/g, '') || generateCouponCode();
      
      // Ensure uniqueness
      const existing = await db.select().from(creatorCoupons).where(eq(creatorCoupons.code, code)).limit(1);
      if (existing.length > 0) {
        code = generateCouponCode() + code.slice(0, 4);
      }
      
      const [coupon] = await db.insert(creatorCoupons).values({
        creatorId,
        code,
        name,
        description,
  ...
  // Update coupon
  app.patch('/api/creator/coupons/:id', isAuthenticated, requireCreator, async (req, res) => {
    try {
      const creatorId = (req as any).user?.id || (req as any).ssoUser?.id;
      ...
      const [coupon] = await db.update(creatorCoupons)
        .set({ ...updates, updatedAt: new Date() })
        .where(and(
          eq(creatorCoupons.id, couponId),
          eq(creatorCoupons.creatorId, creatorId)
        ))
        .returning();
  ...
  // Delete coupon
  app.delete('/api/creator/coupons/:id', isAuthenticated, requireCreator, async (req, res) => {
    try {
      const creatorId = (req as any).user?.id || (req as any).ssoUser?.id;
      const couponId = parseInt(req.params.id);
      
      await db.delete(creatorCoupons)
        .where(and(
          eq(creatorCoupons.id, couponId),
          eq(creatorCoupons.creatorId, creatorId)
        ));
  ...
  // Validate coupon (public endpoint for fans)
  app.post('/api/coupons/validate', isAuthenticated, async (req, res) => {
    try {
      ...
      const [coupon] = await db.select()
        .from(creatorCoupons)
        .where(and(
          eq(creatorCoupons.code, code.toUpperCase()),
          eq(creatorCoupons.creatorId, creatorId),
          eq(creatorCoupons.status, 'active')
        ))
        .limit(1);
      ...
      res.json({
        ...
        coupon: {
          ...
          bundleMonths: coupon.bundleMonths,
        }
      });
    } catch (error) {
  ...
  // Get active promotions for a creator (public)
  app.get('/api/creators/:creatorId/promotions', async (req, res) => {
    try {
      const { creatorId } = req.params;
      const now = new Date();
      ...
      const promotions = await db.select()
        ...
        .where(and(
          eq(creatorPromotions.creatorId, creatorId),
          eq(creatorPromotions.status, 'active'),
          eq(creatorPromotions.isPublic, true),
          lte(creatorPromotions.startsAt, now)
        ))
        .orderBy(desc(creatorPromotions.value));
      ...
      console.error('Failed to get creator promotions:', error);
      res.status(500).json({ error: 'Failed to get promotions' });
    }
  });

  // Get discount analytics for creator
  app.get('/api/creator/discount-analytics', isAuthenticated, requireCreator, async (req, res) => {
    try {
      const creatorId = (req as any).user?.id || (req as any).ssoUser?.id;
      ...
      // Get promotion redemption stats
      const promotionStats = await db.select({
        count: sql<number>`count(*)`,
        totalDiscount: sql<number>`sum(discount_amount_cents)`,
        totalRevenue: sql<number>`sum(final_price_cents)`,
      })
        .from(promotionRedemptions)
        .innerJoin(creatorPromotions, eq(promotionRedemptions.promotionId, creatorPromotions.id))
        .where(and(
          eq(creatorPromotions.creatorId, creatorId),
          gte(promotionRedemptions.redeemedAt, start),
          lte(promotionRedemptions.redeemedAt, end)
        ));
      ...
      const couponStats = await db.select({
        ...
        .from(couponRedemptions)
        .innerJoin(creatorCoupons, eq(couponRedemptions.couponId, creatorCoupons.id))
        .where(and(
          eq(creatorCoupons.creatorId, creatorId),
          gte(couponRedemptions.redeemedAt, start),
          lte(couponRedemptions.redeemedAt, end)
        ));
      
      res.json({
        promotions: promotionStats[0] || { count: 0, totalDiscount: 0, totalRevenue: 0 },
        coupons: couponStats[0] || { count: 0, totalDiscount: 0, totalRevenue: 0 },
        period: { start, end },
      });
    } catch (error) {
