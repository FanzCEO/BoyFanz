...
export function registerRoutes(app: Express) {
  ...
  // Get OAuth settings for all providers
  app.get('/api/admin/oauth-settings', requireAdmin, async (req, res) => {
    try {
      const providers = ['google', 'github', 'facebook', 'twitter', 'discord'];
      ...
      const providerSettings = await Promise.all(
        providers.map(async (provider) => {
          // Check both database settings and environment variables
          const dbSetting = await storage.getSystemSetting(`oauth_${provider}_client_id`);
          const clientIdEnvKey = provider === 'twitter' ? 'TWITTER_CONSUMER_KEY' : `${provider.toUpperCase()}_CLIENT_ID`;
          const clientSecretEnvKey = provider === 'twitter' ? 'TWITTER_CONSUMER_SECRET' : `${provider.toUpperCase()}_CLIENT_SECRET`;

          const clientId = dbSetting?.value || process.env[clientIdEnvKey] || '';
          const hasSecret = !!(await storage.getSystemSetting(`oauth_${provider}_client_secret`))?.value || !!process.env[clientSecretEnvKey];

          return {
            id: provider,
            name: provider.charAt(0).toUpperCase() + provider.slice(1),
            enabled: !!(clientId && hasSecret),
  ...
  // Save OAuth settings for a provider
  app.post('/api/admin/oauth-settings', csrfProtection, requireAdmin, async (req, res) => {
    try {
      const { provider, clientId, clientSecret, enabled } = req.body;

      if (!provider || !['google', 'github', 'facebook', 'twitter', 'discord'].includes(provider)) {
        return res.status(400).json({ error: 'Invalid provider' });
      }

      // Save client ID (can be stored as plain text)
      if (clientId) {
        await storage.upsertSystemSetting({
          key: `oauth_${provider}_client_id`,
          value: clientId,
          type: 'encrypted',
          category: 'security',
      ...
      if (clientSecret && clientSecret !== '********') {
        await storage.upsertSystemSetting({
          key: `oauth_${provider}_client_secret`,
          value: clientSecret,
          type: 'encrypted',
          category: 'security',
      ...
      // Save enabled status
      await storage.upsertSystemSetting({
        key: `oauth_${provider}_enabled`,
        value: String(enabled),
        type: 'boolean',
        category: 'security',
        description: `${provider} OAuth enabled status`,
        isPublic: false,
        isEditable: true,
        updatedBy: req.user!.id,
      });
      ...
      await storage.createAuditLog({
        ...
        targetType: 'oauth_settings',
        targetId: provider,
        diffJson: { provider, enabled },
      });

      res.json({ message: `${provider} OAuth settings saved successfully` });
    } catch (error) {
  ...
  // Test OAuth connection for a provider
  app.post('/api/admin/oauth-settings/test/:provider', csrfProtection, requireAdmin, async (req, res) => {
    try {
      const { provider } = req.params;

      if (!['google', 'github', 'facebook', 'twitter', 'discord'].includes(provider)) {
        return res.status(400).json({ error: 'Invalid provider' });
      }

      // Check if credentials exist
      const clientIdSetting = await storage.getSystemSetting(`oauth_${provider}_client_id`);
      const clientSecretSetting = await storage.getSystemSetting(`oauth_${provider}_client_secret`);

      const clientIdEnvKey = provider === 'twitter' ? 'TWITTER_CONSUMER_KEY' : `${provider.toUpperCase()}_CLIENT_ID`;
      const clientSecretEnvKey = provider === 'twitter' ? 'TWITTER_CONSUMER_SECRET' : `${provider.toUpperCase()}_CLIENT_SECRET`;

      const clientId = clientIdSetting?.value || process.env[clientIdEnvKey];
      const clientSecret = clientSecretSetting?.value || process.env[clientSecretEnvKey];

      if (!clientId || !clientSecret) {
        return res.status(400).json({ error: 'OAuth credentials not configured' });
      }

      // For now, just verify credentials exist - actual OAuth test would require provider-specific API calls
      res.json({
        success: true,
        message: `${provider} OAuth credentials are configured`,
        provider,
      });
    } catch (error) {
  ...
  // Consent management endpoints - no CSRF required for cookie consent (not security-sensitive)
