...
router.post('/invitations', isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    // Email and phone are both optional - creator can just share the QR code/link manually
    ...
    // TODO: In production, insert into coStarInvitations table when schema is ready
    ...
    // Send email/SMS notification to co-star
    if (coStarEmail) {
    ...
    if (coStarPhone) {
      // Send SMS invitation
      const creatorName = (req as any).user?.firstName || 'A creator';
      const smsResult = await smsService.sendCoStarInvitation(coStarPhone, creatorName, inviteUrl);
      if (smsResult.success) {
        console.log(`[CoStar] SMS sent to ${coStarPhone}: ${smsResult.messageId}`);
      } else {
        console.log(`[CoStar] SMS to ${coStarPhone} failed: ${smsResult.error}`);
      }
    }
