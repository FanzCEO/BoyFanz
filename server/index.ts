...
(async () => {
  ...
  // Register category routes for filtering and orientation tabs
  ...
  // Setup FanzSSO centralized authentication
  setupFanzSSO(app, {
    platformId: 'boyfanz',
...
// Initialize Service Orchestration Engine
  ...
  server.listen(port, "127.0.0.1", async () => {
    ...
    // Start local bots AFTER server is listening (prevents double-start on hot reload)
    try {
      const { startAllLocalBots } = await import('./bots/localBots');
      await startAllLocalBots();
      logger.info('Local bots started successfully');
    } catch (err) {
      logger.error({ error: err }, '[LocalBots] Failed to start local bots');
    }
  });
