...
const JobStatusCard = ({ job }: { job: MassMessageJob }) => {
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'processing': return <Loader2 className="h-4 w-4 text-orange-500 animate-spin" />;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'processing': return 'bg-orange-500/20 text-orange-700 border-orange-500/30';
