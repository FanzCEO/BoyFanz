  ...
  return (
    ...
    >
      <div className={cn(
        ...
        {/* Ad Banner Space - Left Side */}
        <div className="flex-1 min-w-0 flex items-center">
          <div
            className="hidden lg:flex items-center justify-center h-10 px-4 bg-gradient-to-r from-cyan-500/10 via-purple-500/10 to-pink-500/10 rounded-lg border border-cyan-500/20"
            data-ad-slot="header-banner-left"
          >
            <span className="text-xs text-muted-foreground">Ad Space</span>
          </div>
          {/* Connection indicator - minimal */}
          <div className={cn(
            "ml-2 h-2 w-2 rounded-full hidden md:block",
            isConnected ? "bg-green-500" : connectionState === 'reconnecting' ? "bg-yellow-500 animate-pulse" : "bg-red-500"
          )} title={isConnected ? "Connected" : connectionState === 'reconnecting' ? "Reconnecting..." : "Disconnected"} />
        </div>
