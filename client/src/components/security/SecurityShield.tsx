...
export default function SecurityShield({ className }: { className?: string }) {
  ...
  return (
    <div className={cn('relative overflow-hidden rounded-xl border bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900', className)}>
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-slate-500/10 via-purple-500/10 to-pink-500/10 animate-pulse" />
      ...
      <div className="relative p-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-slate-500 to-blue-600 flex items-center justify-center shadow-lg shadow-slate-500/30">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono uppercase tracking-wider">Security Status: Maximum</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-slate-400">
              {score.toFixed(1)}%
            </p>
        ...
        <div className="space-y-2">
          <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-green-500 via-slate-500 to-blue-500 transition-all duration-1000"
              style={{ width: `${score}%` }}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-700/50">
          ...
          <Button
            variant="ghost"
            size="sm"
            className="text-slate-400 hover:text-cyan-300 hover:bg-slate-500/10"
...
function SecurityLayer({
  ...
  const colorClasses: Record<string, string> = {
    cyan: 'from-slate-500/20 to-slate-600/20 border-slate-500/30 text-slate-400',
    ...
    <Dialog>
      <DialogTrigger asChild>
        <button className={cn(
          ...
          'hover:border-slate-500/50 hover:shadow-lg hover:shadow-slate-500/10 transition-all',
          'cursor-pointer',
          className
        )}>
          <div className="relative">
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 py-12 px-4">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-500/10 border border-slate-500/30 rounded-full">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
            <span className="text-sm font-mono text-slate-400 uppercase tracking-wider">All Systems Operational</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold text-white">
            Your Security is Our
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-400 to-blue-500"> Priority</span>
          </h1>
...
function StatCard({
  ...
  const colorClasses: Record<string, string> = {
    ...
    cyan: 'text-slate-400 bg-slate-500/10 border-slate-500/30',
