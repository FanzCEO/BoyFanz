...
import { ForensicHubReporter } from '../../../shared/forensics/forensicHubReporter';
...
export class WatermarkService {
  ...
  private readonly forensicReporter: ForensicHubReporter;

  constructor() {
    // Initialize Forensic Hub Reporter
    this.forensicReporter = new ForensicHubReporter({
      platformId: 'boyfanz',
      enabled: process.env.FORENSIC_HUB_ENABLED !== 'false',
    });
  }
  ...
  async applyWatermark(mediaId: string, userId: string, s3Key: string): Promise<ForensicSignature> {
    try {
      ...
      // Report to Forensic Command Center
      await this.forensicReporter.reportWatermarkApplied({
        mediaId,
        userId,
        verificationHash: forensicSignature.verificationHash,
        digitalSignature: forensicSignature.metadata.digitalSignature,
        contentHash: forensicSignature.metadata.contentHash,
        contentType: 'image', // TODO: detect from s3Key
        pixelPattern: forensicSignature.forensicMarkers.pixelPattern,
        frequencyDomain: forensicSignature.forensicMarkers.frequencyDomain,
        steganographicData: forensicSignature.forensicMarkers.steganographicData,
        watermarkedAt: forensicSignature.metadata.timestamp,
        originalS3Key: s3Key,
      });
  ...
  async verifyWatermark(mediaId: string): Promise<{
    ...
    try {
      ...
      const isValid = signatureValid && hashValid && !tamperingCheck.detected;

      // Report verification to Forensic Command Center
      await this.forensicReporter.reportVerificationCheck(
      ...
      if (tamperingCheck.detected) {
        await this.forensicReporter.reportTamperingDetected(
          signature.verificationHash,
          tamperingCheck.indicators
        );
      }
