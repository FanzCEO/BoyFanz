...
class FanzDashClient {
  ...
  trackForumTopicCreated(userId: string, topicId: string, categoryId: string, title: string): void {
    this.queueEvent({
      type: 'forum_topic_created',
      category: 'community',
      userId,
      data: { topicId, categoryId, title }
    });
  }

  trackForumReplyCreated(userId: string, replyId: string, topicId: string): void {
    this.queueEvent({
      type: 'forum_reply_created',
      category: 'community',
      userId,
      data: { replyId, topicId }
    });
  }

  trackForumModeration(topicId: string, action: string, reason: string | null, moderatorId: string): void {
    this.queueEvent({
  ...
  trackForumCategoryCreated(userId: string, categoryId: string, name: string, isCreatorForum: boolean): void {
    this.queueEvent({
      type: 'forum_category_created',
      category: 'community',
      userId,
      data: { categoryId, name, isCreatorForum }
    });
  }
