...
export function setupHealthEndpoints(app: Express) {
  // Liveness probe - basic service availability (both paths for compatibility)
  const livenessHandler = async (req: any, res: any) => {
    res.json({
      status: 'alive',
      timestamp: new Date().toISOString(),
      uptime: process.uptime()
  ...
  const readinessHandler = async (req: any, res: any) => {
    try {
  ...
  const detailedHealthHandler = async (req: any, res: any) => {
    try {
      const health = await performHealthCheck();
      res.json(health);
    } catch (error) {
