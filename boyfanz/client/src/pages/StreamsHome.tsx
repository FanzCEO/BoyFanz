...
const StreamsHome = () => {
  ...
  // Production mode - fetch real streams from API
  const { data: streamsResponse, isLoading } = useQuery<{
    streams: StreamWithCreator[];
  }>({
    queryKey: ['/api/streams', { category: selectedCategory, search: searchQuery }],
  });
          ...
          <div className="flex-1 min-w-0">
            ...
            {stream.status === 'live' && (
              <div className="flex items-center gap-1 text-sm text-green-600">
                ...
                Started {formatDistanceToNow(new Date(stream.createdAt || Date.now()), { addSuffix: true })}
              </div>
            )}
          </div>
        </div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold mt-1">
              {streams.reduce((sum, stream) => sum + (stream.viewerCount || 0), 0).toLocaleString()}
            </div>
          </CardContent>
        </Card>
