...
export function HelpCenter() {
  ...
  const { data: categoriesResponse } = useQuery<{ success: boolean; data: { name: string; slug: string; count: number }[] }>({
    queryKey: ['/api/help/wiki/categories'],
    staleTime: 10 * 60 * 1000 // 10 minutes
  });
  const wikiCategories = categoriesResponse?.data || [];
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-red-900/20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <section className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {wikiCategories.slice(0, 12).map((category, index) => {
              ...
              const icons = [Book, PlayCircle, Users, Sparkles, MessageSquare, ThumbsUp, Star, Heart, Users, Book, PlayCircle, Sparkles];
              const IconComponent = icons[index % icons.length];

              return (
                <Link key={category.slug} href={`/help/wiki?category=${category.slug}`}>
                  <Card className="group bg-gray-800/50 border-gray-700 hover:border-red-500/50 transition-all duration-300 hover:scale-105 cursor-pointer h-full">
                    <CardContent className="p-6">
                      <div className={`w-12 h-12 rounded-lg bg-gradient-to-r ${colors[index % colors.length]} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}>
                      ...
                      <p className="text-gray-400 text-sm mb-4">
                        Browse articles about {category.name.toLowerCase()}
                      </p>
            ...
            <div className="text-center mt-8">
              <Link href="/help/wiki">
                <Button variant="outline" className="bg-gray-800 border-gray-700 text-white hover:bg-gray-700">
                  View All {wikiCategories.length} Categories <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </Link>
            </div>
          )}
