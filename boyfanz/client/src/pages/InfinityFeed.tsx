...
export default function InfinityFeed() {
  ...
  const { data: suggestedCreators = [] } = useQuery<SuggestedCreator[]>({
  ...
  const { data: storiesData } = useInfiniteQuery({
    ...
    queryFn: async () => {
      ...
      return response.json() as Promise<StoryGroup[]>;
    },
    getNextPageParam: () => undefined,
    initialPageParam: 1,
    enabled: !!user,
  });
  const storyGroups = storiesData?.pages[0] || [];
          ...
          <CardContent className="p-0">
            ...
            <div className="px-2 py-1 flex items-center justify-around border-t border-red-900/20">
              ...
              <div className="relative">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      variant="ghost"
                      className="flex items-center gap-2 text-gray-400 hover:text-red-400 hover:bg-red-950/30 px-4"
              ...
              <Link href={`/post/${post.id}`}>
                <Button variant="ghost" className="flex items-center gap-2 text-gray-400 hover:text-blue-400 hover:bg-blue-950/30 px-4">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex items-center gap-2 text-gray-400 hover:text-green-400 hover:bg-green-950/30 px-4">
              ...
              <Button
                ...
                className={cn(
                  ...
                  post.isSaved ? "text-yellow-400" : "text-gray-400 hover:text-yellow-400 hover:bg-yellow-950/30"
                )}
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <aside className="hidden lg:block lg:col-span-3 space-y-4">
            <Card className="bg-black/40 backdrop-blur-xl border-red-900/30 sticky top-20">
              <CardContent className="p-4 space-y-1">
                {/* User Profile Quick Access */}
                <Link href={`/creator/${user?.id}`}>
                  <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-red-950/30 transition-colors cursor-pointer">
                ...
                <nav className="space-y-1">
                  {[
                    ...
                    { icon: Tv, label: 'Live Streams', href: '/streams', badge: liveStreams.length || null },
                    { icon: Users, label: 'My Subscriptions', href: '/subscriptions' },
                    { icon: MessageSquare, label: 'Messages', href: '/messages' },
                    { icon: Bookmark, label: 'Saved Posts', href: '/saved' },
                    { icon: Calendar, label: 'Events', href: '/events' },
                    { icon: Crown, label: 'Starz Studio', href: '/starz-studio' },
                ...
                {/* Categories */}
                <p className="text-xs text-gray-500 px-3 py-2 uppercase tracking-wider">Explore</p>
                <nav className="space-y-1">
                  {[
                    { icon: Flame, label: 'Trending', color: 'text-orange-400' },
                    { icon: Star, label: 'Top Creators', color: 'text-yellow-400' },
                    { icon: Sparkles, label: 'New Stars', color: 'text-purple-400' },
                    { icon: Heart, label: 'For You', color: 'text-pink-400' },
                  ].map((item) => (
                    <button
                      key={item.label}
                      className="w-full flex items-center gap-3 p-3 rounded-lg text-gray-300 hover:bg-red-950/30 hover:text-white transition-colors"
                    >
          ...
          <main className="lg:col-span-6 space-y-4">
            {/* Stories Section */}
            <Card className="bg-black/40 backdrop-blur-xl border-red-900/30 overflow-hidden">
            ...
            <Card className="bg-black/40 backdrop-blur-xl border-red-900/30">
              <CardContent className="p-4">
                <div className="flex items-center gap-4">
                  ...
                  >
                    What's on your mind, {user?.firstName || 'Star'}?
                  </button>
                </div>
            ...
            {/* Feed Filter Tabs */}
            <div className="flex items-center gap-2 bg-black/40 backdrop-blur-xl border border-red-900/30 rounded-xl p-1">
              {[
          ...
          <aside className="hidden lg:block lg:col-span-3 space-y-4">
            ...
            <Card className="bg-black/40 backdrop-blur-xl border-red-900/30 sticky top-20">
              <CardContent className="p-4">
                  ...
                  {(trendingTopics.length > 0 ? trendingTopics : [
                    ...
                    <div key={topic.id} className="group cursor-pointer">
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="text-xs text-gray-500">{idx + 1} • {topic.category}</p>
                ...
                <Button variant="ghost" className="w-full mt-4 text-red-400 hover:text-red-300 hover:bg-red-950/30">
                  Show more
                </Button>
              </CardContent>
            </Card>
                  ...
                  {(suggestedCreators.length > 0 ? suggestedCreators : [
                    ...
                    <div key={creator.id} className="flex items-center gap-3">
                      <Link href={`/creator/${creator.id}`}>
                        <Avatar className="h-12 w-12 ring-2 ring-red-500/30 cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        <Link href={`/creator/${creator.id}`}>
                          <div className="flex items-center gap-1 cursor-pointer">
                            <span className="font-medium text-white hover:text-red-400 transition-colors truncate">
                              {creator.displayName}
                            </span>
                            {creator.isVerified && (
