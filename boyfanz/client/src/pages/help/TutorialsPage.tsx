...
import { AITrainingAssistant } from '@/components/tutorials/AITrainingAssistant';
...
export function TutorialsPage() {
  ...
  // Fetch creator training progress (only for creators)
  const { data: trainingProgress } = useQuery<{
    progress: Array<{ tutorialId: string; isCompleted: boolean }>;
    allComplete: boolean;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-green-900/20">
      ...
      {/* Mandatory Training Alert for Creators */}
      {isCreator && trainingProgress && !trainingProgress.allComplete && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
          <Alert className="bg-red-900/30 border-red-500/50">
            <Shield className="h-5 w-5 text-red-400" />
            <AlertTitle className="text-red-400 font-semibold">
              Mandatory Creator Training Required
            </AlertTitle>
            <AlertDescription className="text-gray-300">
              <p className="mb-4">
                You must complete {trainingProgress.totalRequired - trainingProgress.completedCount} more
                mandatory training module(s) before you can upload content or go live.
                This training covers legal requirements (2257 compliance), CoStar verification, and platform best practices.
              </p>
              <div className="flex items-center gap-4 mb-4">
                <Progress
                  value={(trainingProgress.completedCount / trainingProgress.totalRequired) * 100}
                  className="flex-1 h-3"
                />
                <span className="text-sm text-gray-400">
                  {trainingProgress.completedCount}/{trainingProgress.totalRequired} complete
                </span>
              </div>
              <Button
                onClick={() => setLocation('/creator/training')}
      ...
      {/* Training Complete Badge for Creators */}
      {isCreator && trainingProgress?.allComplete && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
          <Alert className="bg-green-900/30 border-green-500/50">
            ...
            <AlertDescription className="text-gray-300">
              You've completed all mandatory creator training. You're certified to create content on BoyFanz!
            </AlertDescription>
          </Alert>
        </div>
      )}
      ...
      {/* AI Training Assistant - Floating */}
      <AITrainingAssistant
        context={{
          currentPage: '/help/tutorials',
          userRole: isCreator ? 'creator' : 'fan'
        }}
        isFloating={true}
      />
    </div>
