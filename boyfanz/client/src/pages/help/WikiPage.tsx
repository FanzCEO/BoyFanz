...
import { WITTLE_BEAR_FOUNDATION } from '@/content/foundation';
...
export function WikiPage() {
  ...
  // Fetch wiki categories with error handling
  const { data: categoriesResponse, isError: categoriesError, refetch: refetchCategories } = useQuery<{ success: boolean; data: WikiCategory[] }>({
  ...
  // Build query string for wiki articles
  const buildWikiUrl = () => {
    ...
    return `/api/help/wiki${queryString ? `?${queryString}` : ''}`;
  };

  // Fetch wiki articles with error handling
  const { data: articlesResponse, isLoading, isError: articlesError, refetch: refetchArticles } = useQuery<{
    success: boolean;
    data: {
    ...
    queryKey: [buildWikiUrl()],
    staleTime: 5 * 60 * 1000, // 5 minutes
    retry: 3,
    retryDelay: (attemptIndex) => Math.min(1000 * 2 ** attemptIndex, 10000),
  });
  const articlesData = articlesResponse?.data || { articles: [], totalCount: 0, hasMore: false };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-red-900/20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <Alert variant="destructive" className="bg-red-900/50 border-red-500/50">
            ...
            <AlertDescription className="flex items-center justify-between">
              <span>Having trouble loading knowledge base content. Please try again.</span>
              <Button
                variant="outline"
                size="sm"
      ...
      {/* Featured: Wittle Bear Foundation */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Card className="bg-gradient-to-br from-pink-900/30 via-gray-900/50 to-amber-900/30 border-pink-500/30 overflow-hidden">
          <CardContent className="p-6 md:p-8">
            <div className="flex flex-col md:flex-row items-start gap-6">
              ...
              <div className="flex-1">
                <Badge className="mb-2 bg-pink-500/20 text-pink-300 border-pink-500/30">Featured</Badge>
                <h3 className="text-2xl font-bold text-white mb-2">
                  {WITTLE_BEAR_FOUNDATION.name}
                </h3>
                <p className="text-pink-200/70 italic mb-3">{WITTLE_BEAR_FOUNDATION.tagline}</p>
                <p className="text-gray-300 mb-4">
                  {WITTLE_BEAR_FOUNDATION.mission.split('\n')[0]}
                </p>
                <div className="flex flex-wrap gap-4 mb-4">
                  {WITTLE_BEAR_FOUNDATION.causes.map((cause, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      <span className={cause.color === 'pink' ? 'text-pink-300' : 'text-amber-300'}>
                        {cause.title}
                      </span>
                    </div>
                  ))}
                ...
                <div className="flex gap-3">
                  <Link href="/help/wiki/about-wittle-bear-foundation">
                    <Button className="bg-pink-600 hover:bg-pink-700">
                      Learn More <ArrowRight className="ml-2 h-4 w-4" />
                    </Button>
                  </Link>
                  <a href={WITTLE_BEAR_FOUNDATION.contact.website} target="_blank" rel="noopener noreferrer">
                    <Button variant="outline" className="border-pink-500/50 text-pink-300 hover:bg-pink-500/10">
                      Visit Foundation
                    </Button>
                  </a>
                </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-3/4">
            ...
            {/* No Results */}
            {articlesData.articles.length === 0 && !isLoading && !articlesError && (
