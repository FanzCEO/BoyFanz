...
interface Compliance2257FormProps {
  onComplete?: () => void;
}

export function Compliance2257Form({ onComplete }: Compliance2257FormProps) {
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [calculatedAge, setCalculatedAge] = useState<number>(0);

  const form = useForm<Verification2257Form>({
    resolver: zodResolver(verification2257Schema) as any,
    defaultValues: {
      legalName: "",
      stageName: "",
      maidenName: "",
  ...
  const handleFileUpload = (field: string) => async (
    ...
    try {
      ...
      const response = await apiRequest("/api/upload/get-url", {
        method: "POST",
        body: {
          filename: file.name,
  ...
  const onSubmit = async (data: Verification2257Form) => {
    try {
      ...
      // Submit to backend
      await apiRequest("/api/compliance/2257-verification", {
        method: "POST",
        body: data,
      });
...
export default Compliance2257Form;
